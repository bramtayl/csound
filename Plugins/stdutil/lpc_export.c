...
{
    csoundMessage(csound, "%s", Str("usage: lpc_export lpc_file cstext-file\n"));
}
...
static int32_t lpc_export(CSOUND *csound, int32_t argc, char **argv)
{
    ...
    str = (char *)mmalloc(csound,hdr.headersize-sizeof(LPHEADER)+4);
    ...
    if (UNLIKELY(coef==NULL)) {
      fclose(inf); fclose(outf); mfree(csound,str); return 3;}
...
{
    int32_t retval = csoundAddUtility(csound, "lpc_export", lpc_export);
    if (!retval) {
      retval =
        csoundSetUtilityDescription(csound, "lpc_export",
