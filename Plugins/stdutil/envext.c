...
#define FIND(MSG)   if (*s == '\0')  \
    if (UNLIKELY(!(--argc) || ((s = *++argv) && *s == '-'))) {          \
      csoundMessage(csound, "%s", MSG); csoundLongJmp(csound, 1); }
...
static void envext_usage(CSOUND *csound, char *mesg, ...)
{
    ...
    csoundMessage(csound,"%s", Str("Usage:\tenvext [-flags] soundfile\n"));
    csoundMessage(csound,"%s", Str("Legal flags are:\n"));
    csoundMessage(csound,"%s", Str("-o fnam\tsound output filename\n"));
    csoundMessage(csound, "%s", Str( "-w time\tSize of window\n"));
    csoundMessage(csound,"%s", Str("flag defaults: envext -onewenv -w0.25\n"));
    va_start(args, mesg);
    csoundErrMsgV(csound, Str("envext: error: "), mesg, args);
    va_end(args);
...
SCsndgetset(CSOUND *csound, SOUNDIN **pp, char *inputfile)
{
    ...
    *pp = p = (SOUNDIN *) mcalloc(csound, sizeof(SOUNDIN));
    p->channel = ALLCHNLS;
    p->skiptime = FL(0.0);
    strNcpy(p->sfname, inputfile, MAXSNDNAME-1);
    if ((infd = sndgetset(csound, p)) == 0) /*open sndfil, do skiptime*/
      return(0);
    p->getframes = p->framesrem;
    dur = (double) p->getframes / p->sr;
    csoundMessage(csound,Str("enveloping %"PRId64" sample frames (%3.1f secs)\n"),
             ...
             double window, char *outname)
{
    ...
    while ((read_in = getsndin(csound,infd,buffer,bufferlen,p)) > 0) {
      max = 0.0;        mxpos = 0;
      min = 0.0;        minpos = 0;
      for (i=0; i<read_in; i++) {
...
{
    int32_t retval = csoundAddUtility(csound, "envext", envext);
    if (!retval) {
      retval =
        csoundSetUtilityDescription(csound, "envext",
