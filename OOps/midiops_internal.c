...
int32_t ctrlinit(CSOUND *csound, CTLINIT *p)
{
    ...
    if (UNLIKELY((nargs & 0x1) == 0)) {
      return csoundInitError(csound, Str("uneven ctrl pairs"));
    }
    else {
      MCHNBLK *chn;
      MYFLT **argp = p->ctrls;
      ...
      do {
        ...
        if (UNLIKELY(ctlno < 0 || ctlno > 127)) {
          return csoundInitError(csound, Str("illegal ctrl no"));
        }
        val = **argp++;
        if (val < FL(0.0) || val > FL(127.0))
          return csoundInitError(csound, Str("Value out of range [0,127]\n"));
...
int32_t cpstmid(CSOUND *csound, CPSTABLE *p)
{
    ...
    if (UNLIKELY((ftp = csoundFTnp2Find(csound, p->tablenum)) == NULL)) {
      return csoundInitError(csound, Str("cpstabm: invalid modulator table"));
    }
    func = ftp->ftable;
...
int32_t imidictl(CSOUND *csound, MIDICTL *p)
{
    ...
    if (UNLIKELY((ctlno = (int32_t)*p->ictlno) < 0 || ctlno > 127))
      return csoundInitError(csound, Str("illegal controller number"));
...
{
    *(p->ichn) = (MYFLT) (csoundGetMidiChannelNumber(p) + 1);
    return OK;
}
...
int32_t pgmassign_(CSOUND *csound, PGMASSIGN *p, int32_t instname)
{
    ...
    if (instname || isstrcod(*p->inst)) {
      MYFLT buf[128];
      strarg2name(csound, (char*) buf, p->inst, "", 1);
    ...
    if (*(p->ipgm) < FL(0.5)) {         /* program <= 0: assign all pgms */
      ...
      if (UNLIKELY(pgm < 0 || pgm > 127)) {
        return csoundInitError(csound, Str("pgmassign: invalid program number"));
      }
      if (!chn) {                           /* on all channels */
        for (chn = 0; chn < 16; chn++)
          csound->m_chnbp[chn]->pgm2ins[pgm] = ins;
...
{
    PRESET_GLOB *q =
      (PRESET_GLOB*)csoundQueryGlobalVariable(csound, "presetGlobals_");
    if (q==NULL) {
      if (UNLIKELY(csoundCreateGlobalVariable(csound, "presetGlobals_",
                                                sizeof(PRESET_GLOB)) != 0))
        return
          csoundInitError(csound, "%s",
                            Str("ctrlpreset: failed to allocate globals"));
      q = (PRESET_GLOB*)csoundQueryGlobalVariable(csound, "presetGlobals_");
      q->max_num = 10;
      q->presets = (int**)mcalloc(csound, 10*sizeof(int*));
    }
    p->q = q;
    return OK;
}
...
int presetctrl_perf(CSOUND *csound, PRESETCTRL *p)
{
    ...
    slot = q->presets[tag];
    if (slot) mfree(csound, slot);
    q->presets[tag] = (int*) mmalloc(csound, sizeof(int)*(p->INOCOUNT));
    slot = q->presets[tag];
    slot[0] = p->INOCOUNT;
    slot[1] = (int)(*p->chnl);
...
int presetctrl1_perf(CSOUND *csound, PRESETCTRL1 *p)
{
    ...
    slot = q->presets[tag];
    if (slot) mfree(csound, slot);
    q->presets[tag] = (int*) mmalloc(csound,
...
int selectctrl_init(CSOUND *csound, SELECTCTRL *p)
{
    PRESET_GLOB *q =
      (PRESET_GLOB*)csoundQueryGlobalVariable(csound, "presetGlobals_");
    if (q==NULL) {
      return csoundInitError(csound, Str("No presets stored"));
    }
    p->q = q;
    return OK;
}
...
int selectctrl_perf(CSOUND *csound, SELECTCTRL *p)
{
    ...
    if (tag>=q->max_num ||NULL==(slot = q->presets[tag])) {
      return csoundPerfError(csound, &p->h, Str("No such preset %d\n"), tag+1);
    }
    {
      int nargs = slot[0];
...
static OENTRY midiops_internal_localops[] = {
    {"aftouch", sizeof(MIDIKMAP), 0, 3, "k", "oh", (SUBR)aftset, (SUBR)aftouch,
     NULL, NULL},
    {"ampmidi", sizeof(MIDIAMP), 0, 1, "i", "io", (SUBR)ampmidi, NULL, NULL,
     NULL},
    {"chanctrl.i", sizeof(CHANCTL), 0, 1, "i", "iioh", (SUBR)ichanctl, NULL,
     NULL, NULL},
    {"chanctrl.k", sizeof(CHANCTL), 0, 3, "k", "iioh", (SUBR)chctlset,
     (SUBR)chanctl, NULL, NULL},
    {"cpsmidi", sizeof(MIDIKMB), 0, 1, "i", "", (SUBR)cpsmidi, NULL, NULL,
     NULL},
    {"cpsmidib.i", sizeof(MIDIKMB), 0, 1, "i", "o", (SUBR)icpsmidib_i, NULL,
     NULL, NULL},
    {"cpsmidib.k", sizeof(MIDIKMB), 0, 3, "k", "o", (SUBR)midibset,
     (SUBR)icpsmidib, NULL, NULL},
    {"cpstmid", sizeof(CPSTABLE), 0, 1, "i", "i", (SUBR)cpstmid, NULL, NULL,
     NULL},
    {"ctlchn", sizeof(CTLIN), 0, 3, "kkk", "oo", (SUBR)ctlin_set, (SUBR)ctlin,
     NULL, NULL},
    {"ctrlinit", sizeof(CTLINIT), 0, 1, "", "im", (SUBR)ctrlinit, NULL, NULL,
     NULL},
    {"ctrlinit.S", sizeof(CTLINITS), 0, 1, "", "Sm", (SUBR)ctrlnameinit, NULL,
     NULL, NULL},
    {"ctrlpreset", sizeof(PRESETCTRL1), 0, 3, "k", "kk[]",
     (SUBR)presetctrl1_init, (SUBR)presetctrl1_perf, NULL},
    {"ctrlpreset", sizeof(PRESETCTRL), 0, 3, "k", "kim", (SUBR)presetctrl_init,
     (SUBR)presetctrl_perf, NULL},
    {"ctrlprint", sizeof(PRINTCTRL), 0, 3, "", "k[]", (SUBR)printctrl_init,
     (SUBR)printctrl, NULL},
    {"ctrlprint.S", sizeof(PRINTCTRL), 0, 3, "", "k[]S", (SUBR)printctrl_init1,
     (SUBR)printctrl, NULL},
    {"ctrlprintpresets", sizeof(PRINTPRESETS), 0, 3, "", "",
     (SUBR)printpresets_init, (SUBR)printpresets_perf, NULL},
    {"ctrlprintpresets.S", sizeof(PRINTPRESETS), 0, 3, "", "S",
     (SUBR)printpresets_init1, (SUBR)printpresets_perf, NULL},
    {"ctrlsave", sizeof(SAVECTRL), 0, 3, "k[]", "im", (SUBR)savectrl_init,
     (SUBR)savectrl_perf, NULL, NULL},
    {"ctrlselect", sizeof(SELECTCTRL), 0, 3, "", "k", (SUBR)selectctrl_init,
     (SUBR)selectctrl_perf, NULL},
    {"massign", sizeof(MASSIGN), 0, 1, "", "iip", (SUBR)massign_p, NULL, NULL,
     NULL},
    {"massign.iS", sizeof(MASSIGNS), 0, 1, "", "iSp", (SUBR)massign_S, NULL,
     NULL, NULL},
    {"midiarp", sizeof(MIDIARP), 0, 3, "kk", "kO", (SUBR)midiarp_set,
     (SUBR)midiarp, NULL, NULL},
    {"midichn", sizeof(MIDICHN), 0, 1, "i", "", (SUBR)midichn, NULL, NULL,
     NULL},
    {"midictrl.i", sizeof(MIDICTL), 0, 1, "i", "ioh", (SUBR)imidictl, NULL,
     NULL},
    {"midictrl.k", sizeof(MIDICTL), 0, 3, "k", "ioh", (SUBR)mctlset,
     (SUBR)midictl, NULL, NULL},
    {"midiin", sizeof(MIDIIN), 0, 3, "kkkk", "", (SUBR)midiin_set, (SUBR)midiin,
     NULL, NULL},
    {"notnum", sizeof(MIDIKMB), 0, 1, "i", "", (SUBR)notnum, NULL, NULL, NULL},
    {"octmidi", sizeof(MIDIKMB), 0, 1, "i", "", (SUBR)octmidi, NULL, NULL,
     NULL},
    {"octmidib.i", sizeof(MIDIKMB), 0, 1, "i", "o", (SUBR)octmidib_i, NULL,
     NULL, NULL},
    {"octmidib.k", sizeof(MIDIKMB), 0, 3, "k", "o", (SUBR)midibset,
     (SUBR)octmidib, NULL, NULL},
    {"pchbend.i", sizeof(MIDIMAP), 0, 1, "i", "jp", (SUBR)ipchbend, NULL, NULL},
    {"pchbend.k", sizeof(MIDIKMAP), 0, 3, "k", "jp", (SUBR)kbndset,
     (SUBR)kpchbend, NULL, NULL},
    {"pchmidi", sizeof(MIDIKMB), 0, 1, "i", "", (SUBR)pchmidi, NULL, NULL,
     NULL},
    {"pchmidib.i", sizeof(MIDIKMB), 0, 1, "i", "o", (SUBR)pchmidib_i, NULL,
     NULL, NULL},
    {"pchmidib.k", sizeof(MIDIKMB), 0, 3, "k", "o", (SUBR)midibset,
     (SUBR)pchmidib, NULL, NULL},
    {"pgmassign", sizeof(PGMASSIGN), 0, 1, "", "iio", (SUBR)pgmassign, NULL,
     NULL, NULL},
    {"pgmassign.S", sizeof(PGMASSIGN), 0, 1, "", "iSo", (SUBR)pgmassign_S, NULL,
     NULL, NULL},
    {"pgmchn", sizeof(PGMIN), 0, 3, "kk", "o", (SUBR)pgmin_set, (SUBR)pgmin,
     NULL, NULL},
    {"polyaft.i", sizeof(MIDICTL), 0, 1, "i", "ioh", (SUBR)imidiaft, NULL, NULL,
     NULL},
    {"polyaft.k", sizeof(MIDICTL), 0, 3, "k", "ioh", (SUBR)maftset,
     (SUBR)midiaft, NULL, NULL},
    {"veloc", sizeof(MIDIMAP), 0, 1, "i", "oh", (SUBR)veloc, NULL, NULL, NULL},
};

LINKAGE_BUILTIN(midiops_internal_localops)
