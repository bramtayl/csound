...
void DOWNset(CSOUND *p, DOWNDAT *downdp, int32_t npts)
{
    ...
    if (downdp->auxch.auxp == NULL || downdp->auxch.size != (uint32_t)nbytes)
      csoundAuxAlloc(p, nbytes, &downdp->auxch);
    downdp->npts = npts;
}
...
void SPECset(CSOUND *p, SPECDAT *specdp, int32_t npts)
{
    ...
    if (specdp->auxch.auxp == NULL || (uint32_t)nbytes != specdp->auxch.size)
      csoundAuxAlloc(p, nbytes, &specdp->auxch);
    specdp->npts = npts;
}
                           ...
                           /* db or mag2) of log freq within each octave */
{
    ...
    if (UNLIKELY(Q <= FL(0.0)))
      return csoundInitError(csound, Str("illegal Q value"));
    if (UNLIKELY(p->dbout < 0 || p->dbout > 3))
      return csoundInitError(csound, Str("unknown dbout code"));
    ...
    if (nocts != dwnp->nocts ||
      ...
      csoundWarning(csound, Str("\thigh cps %7.1f\n\t low cps %7.1f\n"),
                              hicps, locps);
      ...
      if (*p->idsines != FL(0.0)) {      /* if reqd, dsply windowed sines now! */
        dispset(csound, &p->sinwindow, p->sinp, (int32_t) sumk,
      ...
      csoundWarning(csound, Str("\t%d oct analysis window "
      ...
      if (p->disprd) {                      /* if display requested, */
        ...
        dispset(csound, &p->octwindow, (MYFLT *)p->auxch2.auxp,
...
int32_t spectrum(CSOUND *csound, SPECTRUM *p)
{
    ...
    if (p->disprd)                               /* if displays requested,   */
      if (!(--p->dcountdown)) {                  /*   on countdown           */
        ...
        display(csound, &p->octwindow);  /*      & display           */
...
/*         return csoundInitError(csound, Str("Q %4.1f needs %d samples, " */
/*                                              "octdown has just %d"), */
/*                                          Q, windsiz, nsamps); */
/*       } */
/*       else csoundMessage(csound, Str("noctdft: Q %4.1f uses %d of " */
/*                                        "%d samps per octdown\n"), */
/*                                    Q, windsiz, nsamps); */
/*       auxsiz = (nsamps + 2*sumk) * sizeof(MYFLT);/\* calc local space reqd *\/ */
/*       csoundAuxAlloc(csound, (size_t)auxsiz, &p->auxch);
         /\* & alloc auxspace  *\/ */
...
/*         dispset(csound, &p->dwindow, p->sinp, (int32_t) sumk, */
...
int32_t spdspset(CSOUND *csound, SPECDISP *p)
{
    ...
    if (UNLIKELY(p->wsig->auxch.auxp==NULL)) {
      return csoundInitError(csound, Str("specdisp: not initialised"));
    }
    if (UNLIKELY((p->timcount = (int32_t)(CS_EKR * *p->iprd)) <= 0)) {
      return csoundInitError(csound, Str("illegal iperiod"));
    }
    if (!(p->dwindow.windid)) {
      SPECDAT *specp = p->wsig;
      ...
      dispset(csound, &p->dwindow, (MYFLT*) specp->auxch.auxp,
...
int32_t specdisp(CSOUND *csound, SPECDISP *p)
{
    ...
    if (!(--p->countdown)) {            /* on countdown     */
      display(csound, &p->dwindow);     /*    display spect */
...
/*       fdp->altarg = "X-corr"; */
    ...
    if ((rolloff = *p->irolloff) == 0.0 || rolloff == 1.0 || nptls == 1) {
      ...
      if (UNLIKELY(*--fltp < FL(0.0))) {
        return csoundInitError(csound, Str("per oct rolloff too steep"));
      }
      p->rolloff = 1;
    }
    ...
    csoundWarning(csound, Str("specptrk: %d freqs, %d%s ptls at "),
                    (int32_t)nfreqs, (int32_t)nptls, inc==2 ? Str(" odd") : "");
    for (nn = 0; nn < nptls; nn++)
      csoundWarning(csound, "\t%d", p->pdist[nn]);
    if (p->rolloff) {
      csoundWarning(csound, Str("\n\t\trolloff vals:"));
      for (nn = 0; nn < nptls; nn++)
        csoundWarning(csound, "\t%4.2f", p->pmult[nn]);
    }
...
int32_t spadmset(CSOUND *csound, SPECADDM *p)
{
    ...
    if (UNLIKELY((npts = inspec1p->npts) != inspec2p->npts))
      ...
      return csoundInitError(csound, Str("inputs have different sizes"));
    if (UNLIKELY(inspec1p->ktimprd != inspec2p->ktimprd))
      /*                time period */
      return csoundInitError(csound, Str("inputs have diff. time periods"));
    ...
    if (UNLIKELY(inspec1p->dbout != inspec2p->dbout))
      /*                and db type */
      return csoundInitError(csound, Str("inputs have different amptypes"));
...
int32_t spsclset(CSOUND *csound, SPECSCAL *p)
{
    ...
    if ((npts = inspecp->npts) != outspecp->npts) {  /* if size has changed,   */
      ...
      csoundAuxAlloc(csound, (int32_t)npts * 2 * sizeof(MYFLT), &p->auxch);
    }
    ...
    if (UNLIKELY((ftp=csoundFTFind(csound, p->ifscale)) == NULL)) {
      /* if fscale given,        */
      return csoundInitError(csound, Str("missing fscale table"));
    }
    else {
      int32_t nn; // = npts;
      int32_t phs = 0;
    ...
    if ((p->thresh = (int32_t)*p->ifthresh) &&
        (ftp=csoundFTFind(csound, p->ifthresh)) != NULL) {
      /* if fthresh given,       */
      int32_t nn; // = npts;
      int32_t phs = 0;
      int32_t inc = (int32_t)PHMASK / npts;
...
int32_t spfilset(CSOUND *csound, SPECFILT *p)
{
    ...
    outspecp->dbout = inspecp->dbout;
    outspecp->downsrcp = inspecp->downsrcp;
    if (UNLIKELY((ftp=csoundFTFind(csound, p->ifhtim)) == NULL)) {
      /* if fhtim table given,    */
      return csoundInitError(csound, Str("missing htim ftable"));
    }
    {
      int32_t nn;
      int32_t phs = 0;
    ...
    csoundWarning(csound, Str("coef range: %6.3f - %6.3f\n"),
...
{ "spectrum", S(SPECTRUM),_QQ, 3, "w", "kiiiqoooo",
                                   (SUBR)spectset,(SUBR)spectrum, NULL, NULL},
{ "spectrum", S(SPECTRUM),_QQ, 3, "w", "aiiiqoooo",
                                   (SUBR)spectset,(SUBR)spectrum, NULL, NULL},
{ "specaddm", S(SPECADDM),_QQ, 3, "w", "wwp", (SUBR)spadmset,  (SUBR)specaddm, NULL, NULL},
{ "specdiff", S(SPECDIFF),_QQ, 3, "w", "w",   (SUBR)spdifset,  (SUBR)specdiff, NULL, NULL},
{ "specscal", S(SPECSCAL),_QQ, 3, "w", "wii", (SUBR)spsclset,  (SUBR)specscal, NULL, NULL},
{ "spechist", S(SPECHIST),_QQ, 3, "w", "w",   (SUBR)sphstset,  (SUBR)spechist, NULL, NULL},
{ "specfilt", S(SPECFILT),_QQ, 3, "w", "wi",  (SUBR)spfilset,  (SUBR)specfilt, NULL, NULL},
{ "specptrk", S(SPECPTRK),_QQ, 3, "kk", "wkiiiiiioqooo",
                                             (SUBR)sptrkset,(SUBR)specptrk, NULL, NULL},
{ "specsum",  S(SPECSUM), _QQ, 3, "k", "wo",  (SUBR)spsumset,  (SUBR)specsum, NULL, NULL},
{ "specdisp", S(SPECDISP),_QQ, 3, "",  "wio", (SUBR)spdspset,  (SUBR)specdisp, NULL, NULL},
{ "pitch", S(PITCH),   0,  3,    "kk", "aiiiiqooooojo",
                                             (SUBR)pitchset, (SUBR)pitch, NULL, NULL},
{ "maca", S(SUM),      0,  3,  "a", "y",    (SUBR)macset, (SUBR)maca, NULL, NULL},
{ "mac", S(SUM),       0,  3,  "a", "Z",    (SUBR)macset, (SUBR)mac, NULL, NULL},
{ "clockon", S(CLOCK), 0,  3,  "",  "i",    (SUBR)clockset, (SUBR)clockon, NULL, NULL},
{ "clockoff", S(CLOCK),0,  3,  "",  "i",    (SUBR)clockset, (SUBR)clockoff, NULL, NULL},
{ "readclock", S(CLKRD),0, 1,  "i", "i",    (SUBR)clockread, NULL, NULL, NULL},
{ "readscratch", S(SCRATCHPAD),0, 1, "i", "o", (SUBR)scratchread, NULL, NULL, NULL},
{ "writescratch", S(SCRATCHPAD),0, 1, "", "io", (SUBR)scratchwrite, NULL, NULL, NULL},
{ "pitchamdf",S(PITCHAMDF),0,3,"kk","aiioppoo",
                                     (SUBR)pitchamdfset, (SUBR)pitchamdf, NULL, NULL},
{ "hsboscil",S(HSBOSC), TR, 3, "a", "kkkiiioo",

                                    (SUBR)hsboscset,(SUBR)hsboscil, NULL, NULL},
{ "phasorbnk", S(PHSORBNK),0,3,"a", "xkio",
                                (SUBR)phsbnkset, (SUBR)phsorbnk, NULL, NULL},
{ "phasorbnk.k", S(PHSORBNK),0,3,"k", "xkio",
                                (SUBR)phsbnkset, (SUBR)kphsorbnk, NULL, NULL},
{ "adsynt",S(HSBOSC), TR, 3,  "a", "kkiiiio", (SUBR)adsyntset, (SUBR)adsynt, NULL, NULL},
{ "mpulse", S(IMPULSE), 0, 3,  "a", "kko",
                                    (SUBR)impulse_set, (SUBR)impulse, NULL, NULL},
{ "lpf18", S(LPF18), 0, 3,  "a", "axxxo",  (SUBR)lpf18set, (SUBR)lpf18db, NULL, NULL},
{ "waveset", S(BARRI), 0, 3,  "a", "ako",  (SUBR)wavesetset, (SUBR)waveset, NULL, NULL},
{ "pinkish", S(PINKISH), 0, 3, "a", "xoooo", (SUBR)pinkset, (SUBR)pinkish, NULL, NULL},
{ "noise",  S(VARI), 0, 3,  "a", "xk",   (SUBR)varicolset, (SUBR)varicol, NULL, NULL},
{ "transeg", S(TRANSEG),0, 3,  "k", "iiim",
                                           (SUBR)trnset,(SUBR)ktrnseg, NULL, NULL},
{ "transeg.a", S(TRANSEG),0, 3,  "a", "iiim",
                                           (SUBR)trnset,(SUBR)trnseg, NULL, NULL},
{ "transegb", S(TRANSEG),0, 3,  "k", "iiim",
                              (SUBR)trnset_bkpt,(SUBR)ktrnseg,(SUBR)NULL, NULL},
{ "transegb.a", S(TRANSEG),0, 3,  "a", "iiim",
                              (SUBR)trnset_bkpt,(SUBR)trnseg, NULL, NULL},
{ "transegr", S(TRANSEG),0, 3, "k", "iiim",
                              (SUBR)trnsetr,(SUBR)ktrnsegr,(SUBR)NULL, NULL},
{ "transegr.a", S(TRANSEG),0, 3, "a", "iiim",
                              (SUBR)trnsetr,(SUBR)trnsegr, NULL, NULL},
{ "clip", S(CLIP),      0, 3,  "a", "aiiv", (SUBR)clip_set, (SUBR)clip, NULL, NULL},
{ "cpuprc", S(CPU_PERC),0, 1,     "",     "Si",   (SUBR)cpuperc_S, NULL, NULL, NULL},
{ "maxalloc", S(CPU_PERC),0, 1,   "",     "Si",   (SUBR)maxalloc_S, NULL, NULL, NULL},
{ "cpuprc", S(CPU_PERC),0, 1,     "",     "ii",   (SUBR)cpuperc, NULL, NULL, NULL},
{ "maxalloc", S(CPU_PERC),0, 1,   "",     "ii",   (SUBR)maxalloc, NULL, NULL, NULL},
{ "active", 0xffff, 0, 0, "", "", NULL, NULL, NULL, NULL},
{ "active.iS", S(INSTCNT),0,1,    "i",    "Soo",   (SUBR)instcount_S, NULL, NULL, NULL},
{ "active.kS", S(INSTCNT),0,2,    "k",    "Soo",   NULL, (SUBR)instcount_S, NULL, NULL},
{ "active.i", S(INSTCNT),0,1,     "i",    "ioo",   (SUBR)instcount, NULL, NULL, NULL},
{ "active.k", S(INSTCNT),0,2,     "k",    "koo",   NULL, (SUBR)instcount, NULL, NULL},
{ "p.i", S(PFUN),        0,1,     "i",    "i",     (SUBR)pfun, NULL, NULL, NULL},
{ "p.k", S(PFUNK),       0,3,     "k",    "k",
                                          (SUBR)pfunk_init, (SUBR)pfunk, NULL, NULL},
{ "mute", S(MUTE), 0,1,           "",      "So",   (SUBR)mute_inst_S, NULL, NULL, NULL},
{ "mute.i", S(MUTE), 0,1,         "",      "io",   (SUBR)mute_inst, NULL, NULL, NULL},
{ "median", S(MEDFILT), 0, 3,     "a", "akio", (SUBR)medfiltset, (SUBR)medfilt, NULL, NULL},
{ "mediank", S(MEDFILT), 0,3,     "k", "kkio", (SUBR)medfiltset, (SUBR)kmedfilt, NULL, NULL},
};
