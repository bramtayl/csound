...
static int32_t mtable_i(CSOUND *csound,MTABLEI *p)
{
    ...
    if (UNLIKELY((ftp = csoundFTnp2Find(csound, p->xfn)) == NULL)) {
      return csoundInitError(csound, Str("vtablei: incorrect table number"));
    }
    table = ftp->ftable;
    nargs = p->INOCOUNT-4;
    if (*p->ixmode)
    ...
    if (*p->kinterp) {
      ...
      for (j=0; j < nargs; j++) {
        if (UNLIKELY((indx + 1) * nargs + j >= ftp->flen + 1)) {
          return csoundInitError(csound, Str("vtablei: reading past end of table"));
        }
...
static int32_t mtable_set(CSOUND *csound,MTABLE *p) /*  mtab by G.Maldonado */
{
    FUNC *ftp;
    if (UNLIKELY((ftp = csoundFTnp2Find(csound, p->xfn)) == NULL)) {
      return csoundInitError(csound, Str("vtable: incorrect table number"));
    }
    p->ftable = ftp->ftable;
...
static int32_t mtab_i(CSOUND *csound,MTABI *p)
{
    ...
    if (UNLIKELY((ftp = csoundFTnp2Find(csound, p->xfn)) == NULL)) {
      return csoundInitError(csound, Str("vtabi: incorrect table number"));
    }
    table = ftp->ftable;
    nargs = p->INOCOUNT-2;
...
static int32_t mtab_set(CSOUND *csound,MTAB *p)     /* mtab by G.Maldonado */
{
    FUNC *ftp;
    if (UNLIKELY((ftp = csoundFTnp2Find(csound, p->xfn)) == NULL)) {
      return csoundInitError(csound, Str("vtab: incorrect table number"));
    }
    p->ftable = ftp->ftable;
...
static int32_t mtablew_i(CSOUND *csound,MTABLEIW *p)
{
    ...
    if (UNLIKELY((ftp = csoundFTnp2Find(csound, p->xfn)) == NULL)) {
      return csoundInitError(csound, Str("vtablewi: incorrect table number"));
    }
    table = ftp->ftable;
    nargs = p->INOCOUNT-3;
    if (*p->ixmode)
...
static int32_t mtablew_set(CSOUND *csound,MTABLEW *p)   /* mtabw by G.Maldonado */
{
    FUNC *ftp;
    if (UNLIKELY((ftp = csoundFTnp2Find(csound, p->xfn)) == NULL)) {
      return csoundInitError(csound, Str("vtablew: incorrect table number"));
    }
    p->ftable = ftp->ftable;
...
static int32_t mtabw_i(CSOUND *csound, MTABIW *p)
{
    ...
    if (UNLIKELY((ftp = csoundFTnp2Find(csound, p->xfn)) == NULL)) {
      return csoundInitError(csound, Str("vtabwi: incorrect table number"));
    }
    table = ftp->ftable;
    nargs = p->INOCOUNT-2;
    indx = (int64_t) *p->xndx;
...
static int32_t vadd_i(CSOUND *csound, VECTOROPI *p)
{
    ...
    ftp = csoundFTnp2Find(csound, p->ifn);
    if (UNLIKELY(ftp == NULL))  {
      return csoundInitError(csound,
    ...
    if (UNLIKELY(elements > len))  {
      elements = len;
      csoundWarning(csound,Str("vadd_i: ifn length exceeded"));
    }
...
static int32_t vaddk(CSOUND *csound, VECTOROP *p)
{
    ...
    if (UNLIKELY(elements > len))  {
      ...
      if (UNLIKELY((int32_t) *p->kverbose != 0))
        csoundWarning(csound,Str("vadd: ifn1 length exceeded"));
    }
    for (i = 0; i < elements; i++)
      vector[i] += value;
    return OK;
...
static int32_t vmult_i(CSOUND *csound, VECTOROPI *p)
{
    ...
    ftp = csoundFTnp2Find(csound, p->ifn);
    if (UNLIKELY(ftp == NULL))  {
      return csoundInitError(csound,Str("vadd_i: invalid table number %i"),
                               (int32_t) *p->ifn);
    }
    vector = ftp->ftable;
    len = (int32) ftp->flen;
    ...
    if (UNLIKELY(elements > len))  {
      elements = len;
      csoundWarning(csound,Str("vmult_i: ifn length exceeded"));
    }
    for (i = 0; i < elements; i++)
      vector[i] *= value;
    return OK;
...
static int32_t vmultk(CSOUND *csound, VECTOROP *p)
{
    ...
    if (UNLIKELY(elements > len))  {
      elements = len;
      if (UNLIKELY((int32_t) *p->kverbose != 0))
        csoundWarning(csound,Str("vmult: ifn1 length exceeded"));
    }
    for (i = 0; i < elements; i++)
      vector[i] *= value;
    return OK;
...
static int32_t vpow_i(CSOUND *csound, VECTOROPI *p)
{
    ...
    ftp = csoundFTnp2Find(csound, p->ifn);
    if (UNLIKELY(ftp == NULL))  {
      return csoundInitError(csound,Str("vpow_i: invalid table number %i"),
                               (int32_t) *p->ifn);
    }
    vector = ftp->ftable;
    len = (int32) ftp->flen;
    ...
    if (UNLIKELY(elements > len))  {
      elements = len;
      csoundWarning(csound,Str("vpow_i: ifn length exceeded"));
    }
    for (i = 0; i < elements; i++)
...
static int32_t vpowk(CSOUND *csound, VECTOROP *p)
{
    ...
    if (UNLIKELY(elements > len))  {
      elements = len;
      if (UNLIKELY((int32_t) *p->kverbose != 0))
        csoundWarning(csound,Str("vpow: ifn1 length exceeded"));
    }
    for (i = 0; i < elements; i++)
      vector[i] = POWER(vector[i], value);
    return OK;
...
static int32_t vexp_i(CSOUND *csound, VECTOROPI *p)
{
    ...
    ftp = csoundFTnp2Find(csound, p->ifn);
    if (UNLIKELY(ftp == NULL))  {
      return csoundInitError(csound,Str("vexp_i: invalid table number %i"),
                               (int32_t) *p->ifn);
    }
    vector = ftp->ftable;
    len = (int32) ftp->flen;
    ...
    if (UNLIKELY(elements > len))  {
      elements = len;
      csoundWarning(csound,Str("vexp_i: ifn length exceeded"));
    }
    for (i = 0; i < elements; i++)
...
static int32_t vexpk(CSOUND *csound, VECTOROP *p)
{
    ...
    if (UNLIKELY(elements > len))  {
      elements = len;
      if (UNLIKELY((int32_t) *p->kverbose != 0))
        csoundWarning(csound,Str("vexp: ifn1 length exceeded"));
    }
    for (i = 0; i < elements; i++)
      vector[i] += POWER(value, vector[i]);
    return OK;
...
static int32_t vectorsOp_set(CSOUND *csound, VECTORSOP *p)
{
    FUNC        *ftp1, *ftp2;
/*     if (*p->ifn1 == *p->ifn2)
       csoundWarning(csound, Str("vectorsop: ifn1 = ifn2."));*/
    ftp1 = csoundFTnp2Find(csound, p->ifn1);
    ftp2 = csoundFTnp2Find(csound, p->ifn2);
...
static int32_t vcopy(CSOUND *csound,VECTORSOP *p)
{
    ...
    if (UNLIKELY(elements > len1))  {
      elements = len1;
      if (UNLIKELY((int32_t) *p->kverbose != 0))
        csoundWarning(csound,Str("vcopy: ifn1 length exceeded"));
    }
    ...
    if (UNLIKELY(elements > len2)) {
      if (UNLIKELY((int32_t) *p->kverbose != 0))
        csoundWarning(csound,Str("vcopy: ifn2 length exceeded"));
      n = len2;
    }
    else n = elements;
    i = 0;
...
/*     if (*p->ifn1 == *p->ifn2)
       csoundWarning(csound, Str("vcopy_i: ifn1 = ifn2."));*/
    vector1 = ftp1->ftable;
    vector2 = ftp2->ftable;
    ...
    if (UNLIKELY(elements > len1))  {
      elements = len1;
      csoundWarning(csound,Str("vcopy_i: ifn1 length exceeded"));
    }
/*     elements = (elements < len1 ? elements : len1);*/
    if (srcoffset < 0) {
      /*int32_t   i, */n = -srcoffset;
...
/*     n = (elements < len2 ? elements : len2);*/
    if (UNLIKELY(elements > len2)) {
      csoundWarning(csound,Str("vcopy_i: ifn2 length exceeded"));
      n = len2;
    }
    else n = elements;
    i = 0;
...
static int32_t vaddvk(CSOUND *csound,VECTORSOP *p)
{
    ...
    if (UNLIKELY(elements > len1))  {
      elements = len1;
      if (UNLIKELY((int32_t) *p->kverbose != 0))
        csoundWarning(csound,Str("vaddv: ifn1 length exceeded"));
    }
/*     elements = (elements < len1 ? elements : len1);*/
    if (srcoffset < 0) {
      /*int32_t i, n = -srcoffset; */
    ...
    if (UNLIKELY(elements > len2)) {
      if (UNLIKELY((int32_t) *p->kverbose != 0))
        csoundWarning(csound,Str("vaddv: ifn2 length exceeded"));
      n = len2;
    }
    else n = elements;
    i = 0;
...
static int32_t vaddv_i(CSOUND *csound, VECTORSOPI *p)
{
    ...
    /* if (*p->ifn1 == *p->ifn2)
       csoundWarning(csound, Str("vaddv_i: ifn1 = ifn2."));*/
    vector1 = ftp1->ftable;
    vector2 = ftp2->ftable;
    len1 = (int32) ftp1->flen+1;
    len2 = (int32) ftp2->flen+1;
    ...
    if (UNLIKELY(elements > len1))  {
      elements = len1;
      csoundWarning(csound,Str("vaddv_i: ifn1 length exceeded"));
    }
    /*elements = (elements < len1 ? elements : len1);*/
    if (srcoffset < 0) {
      /*int32_t   i, n = -srcoffset; */
    ...
    /*n = (elements < len2 ? elements : len2);*/
    if (UNLIKELY(elements > len2)) {
      csoundWarning(csound,Str("vaddv_i: ifn2 length exceeded"));
      n = len2;
    }
    else n = elements;
    for (i = 0; i < n; i++)
...
static int32_t vsubvk(CSOUND *csound,VECTORSOP *p)
{
    ...
    if (UNLIKELY(elements > len1))  {
      elements = len1;
      if (UNLIKELY((int32_t) *p->kverbose != 0))
        csoundWarning(csound,Str("vsubv: ifn1 length exceeded"));
    }
        /* elements = (elements < len1 ? elements : len1);*/
    if (srcoffset < 0) {
        i = -srcoffset;
    ...
    if (UNLIKELY(elements > len2)) {
          if (UNLIKELY((int32_t) *p->kverbose != 0))
        csoundWarning(csound,Str("vsubv: ifn2 length exceeded"));
      n = len2;
    }
    else n = elements;
    i = 0;
...
/*     if (*p->ifn1 == *p->ifn2)
       csoundWarning(csound, Str("vsubv_i: ifn1 = ifn2."));*/
    vector1 = ftp1->ftable;
    vector2 = ftp2->ftable;
    len1 = (int32) ftp1->flen+1;
    len2 = (int32) ftp2->flen+1;
    ...
    if (UNLIKELY(elements > len1))  {
      elements = len1;
      csoundWarning(csound,Str("vsubv_i: ifn1 length exceeded"));
    }
    /* elements = (elements < len1 ? elements : len1); */
    if (srcoffset < 0) {
          n = -srcoffset;
      ...
      vector2 += srcoffset;
        ...
        /* n = (elements < len2 ? elements : len2); */
    if (UNLIKELY(elements > len2)) {
      csoundWarning(csound,Str("vsubv_i: ifn2 length exceeded"));
      n = len2;
    }
    else n = elements;
    i = 0;
...
static int32_t vmultvk(CSOUND *csound,VECTORSOP *p)
{
    ...
    if (UNLIKELY(elements > len1))  {
      elements = len1;
      if (UNLIKELY((int32_t) *p->kverbose != 0))
        csoundWarning(csound,Str("vmultv: ifn1 length exceeded"));
    }
    /*elements = (elements < len1 ? elements : len1);*/
    if (srcoffset < 0) {
      i = -srcoffset;
    ...
    /*n = (elements < len2 ? elements : len2);*/
      if (UNLIKELY(elements > len2)) {
        if (UNLIKELY((int32_t) *p->kverbose != 0))
        csoundWarning(csound,Str("vmultv: ifn2 length exceeded"));
      n = len2;
    }
    else n = elements;
    i = 0;
...
/*     if (*p->ifn1 == *p->ifn2)
       csoundWarning(csound, Str("vmultv_i: ifn1 = ifn2."));*/
    vector1 = ftp1->ftable;
    vector2 = ftp2->ftable;
    len1 = (int32) ftp1->flen+1;
    ...
    if (UNLIKELY(elements > len1))  {
      elements = len1;
      csoundWarning(csound,Str("vmultv_i: ifn1 length exceeded"));
    }
    /* elements = (elements < len1 ? elements : len1);*/
    if (srcoffset < 0) {
      /*int32_t   i, */n = -srcoffset;
    ...
    /*n = (elements < len2 ? elements : len2);*/
    if (UNLIKELY(elements > len2)) {
      csoundWarning(csound,Str("vmultv_i: ifn2 length exceeded"));
      n = len2;
    }
    else n = elements;
    i = 0;
...
static int32_t vdivvk(CSOUND *csound,VECTORSOP *p)
{
    ...
    if (UNLIKELY(elements > len1))  {
      elements = len1;
      if (UNLIKELY((int32_t) *p->kverbose != 0))
        csoundWarning(csound,Str("vdivv: ifn1 length exceeded"));
    }
        /* elements = (elements < len1 ? elements : len1); */
    if (srcoffset < 0) {
      i = -srcoffset;
    ...
    if (UNLIKELY(elements > len2)) {
      if (UNLIKELY((int32_t) *p->kverbose != 0))
        csoundWarning(csound,Str("vdivv: ifn2 length exceeded"));
      n = len2;
    }
    else n = elements;
    i = 0;
...
static int32_t vdivv_i(CSOUND *csound, VECTORSOPI *p)
{
    ...
    if (UNLIKELY(ftp1 == NULL)) {
     ...
     /*if (*p->ifn1 == *p->ifn2)
       csoundWarning(csound, Str("vdivv_i: ifn1 = ifn2."));*/
    vector1 = ftp1->ftable;
    vector2 = ftp2->ftable;
    len1 = (int32) ftp1->flen+1;
    len2 = (int32) ftp2->flen+1;
    ...
    if (UNLIKELY(elements > len1))  {
      elements = len1;
      csoundWarning(csound,Str("vdivv_i: ifn1 length exceeded"));
    }
    /* elements = (elements < len1 ? elements : len1); */
    if (srcoffset < 0) {
      n = -srcoffset;
    ...
    /*n = (elements < len2 ? elements : len2);*/
    if (UNLIKELY(elements > len2)) {
      csoundWarning(csound,Str("vdivv_i: ifn2 length exceeded"));
      n = len2;
    }
    else n = elements;
    i = 0;
...
static int32_t vpowvk(CSOUND *csound,VECTORSOP *p)
{
    ...
    if (UNLIKELY(elements > len1))  {
      elements = len1;
      if (UNLIKELY((int32_t) *p->kverbose != 0))
        csoundWarning(csound,Str("vpowv: ifn1 length exceeded"));
    }
    /*elements = (elements < len1 ? elements : len1);*/
    if (srcoffset < 0) {
      i = -srcoffset;
    ...
    if (UNLIKELY(elements > len2)) {
      if (UNLIKELY((int32_t) *p->kverbose != 0))
        csoundWarning(csound,Str("vpowv: ifn2 length exceeded"));
      n = len2;
    }
    else n = elements;
    i = 0;
...
static int32_t vpowv_i(CSOUND *csound, VECTORSOPI *p)
{
    ...
    /*if (*p->ifn1 == *p->ifn2)
      csoundWarning(csound, Str("vpowv_i: ifn1 = ifn2."));*/
    vector1 = ftp1->ftable;
    vector2 = ftp2->ftable;
    len1 = (int32) ftp1->flen+1;
    len2 = (int32) ftp2->flen+1;
    ...
    if (UNLIKELY(elements > len1))  {
      elements = len1;
      csoundWarning(csound,Str("vpowv_i: ifn1 length exceeded"));
    }
    /*elements = (elements < len1 ? elements : len1);*/
    if (srcoffset < 0) {
      /*int32_t   i, */n = -srcoffset;
    ...
    /*n = (elements < len2 ? elements : len2);*/
    if (UNLIKELY(elements > len2)) {
      csoundWarning(csound,Str("vpowv_i: ifn2 length exceeded"));
      n = len2;
    }
    else n = elements;
    i = 0;
...
static int32_t vexpvk(CSOUND *csound,VECTORSOP *p)
{
    ...
    if (UNLIKELY(elements > len1))  {
      elements = len1;
      if (UNLIKELY((int32_t) *p->kverbose != 0))
        csoundWarning(csound,Str("vexpv: ifn1 length exceeded"));
    }
        /* elements = (elements < len1 ? elements : len1); */
    if (srcoffset < 0) {
      i = -srcoffset;
    ...
    if (UNLIKELY(elements > len2)) {
      if (UNLIKELY((int32_t) *p->kverbose != 0))
        csoundWarning(csound,Str("vexpv: ifn2 length exceeded"));
      n = len2;
    }
    else n = elements;
    i = 0;
...
static int32_t vexpv_i(CSOUND *csound, VECTORSOPI *p)
{
    ...
    /*if (*p->ifn1 == *p->ifn2)
      csoundWarning(csound, Str("vexpv_i: ifn1 = ifn2."));*/
    vector1 = ftp1->ftable;
    vector2 = ftp2->ftable;
    len1 = (int32) ftp1->flen+1;
    len2 = (int32) ftp2->flen+1;
    ...
    if (UNLIKELY(elements > len1))  {
      elements = len1;
      csoundWarning(csound,Str("vexpv_i: ifn1 length exceeded"));
    }
    /* elements = (elements < len1 ? elements : len1); */
    if (srcoffset < 0) {
      n = -srcoffset;
    ...
    /*n = (elements < len2 ? elements : len2);*/
    if (UNLIKELY(elements > len2)) {
      csoundWarning(csound,Str("vexpv_i: ifn2 length exceeded"));
      n = len2;
    }
    else n = elements;
    i = 0;
...
/*     if (*p->ifn1 == *p->ifn2)
    ...
    if (UNLIKELY(elements > len1))  {
      elements = len1;
      csoundWarning(csound,Str("vmap: ifn1 length exceeded"));
    }
/*     elements = (elements < len1 ? elements : len1);*/
    if (srcoffset < 0) {
      /*int32_t   i, */n = -srcoffset;
...
/*     n = (elements < len2 ? elements : len2);*/
    if (UNLIKELY(elements > len2)) {
      csoundWarning(csound,Str("vmap: ifn2 length exceeded"));
      n = len2;
    }
    else n = elements;
    i = 0;
...
{
    FUNC        *ftp;
    if (UNLIKELY((ftp = csoundFTnp2Finde(csound,p->ifn)) != NULL)) {
      p->vector = ftp->ftable;
      p->elements = (int32_t) *p->ielements;
    }
    else return NOTOK;
    if (UNLIKELY(p->elements > (int32_t)ftp->flen )) {
      return csoundInitError(csound, Str("vectorop: invalid num of elements"));
    }
    return OK;
}
...
static int32_t vport_set(CSOUND *csound,VPORT *p)
{
    ...
    else return csoundInitError(csound, Str("vport: invalid table"));
    if (LIKELY(*p->ifnInit)) {
      if (LIKELY((ftp = csoundFTnp2Find(csound,p->ifnInit)) != NULL)) {
        vecInit = ftp->ftable;
        if (UNLIKELY(elements > (int32_t)ftp->flen) )
          return csoundInitError(csound, Str("vport: invalid init table length"
                                               " or num of elements"));
      }
      else return csoundInitError(csound, Str("vport: invalid init table"));
    }
    if (p->auxch.auxp == NULL)
      csoundAuxAlloc(csound, elements * sizeof(MYFLT), &p->auxch);
...
static int32_t vrandh_set(CSOUND *csound,VRANDH *p)
{
    ...
    if (*p->iseed >= FL(0.0)) {                       /* new seed:*/
      ...
      if ((ftp = csoundFTnp2Find(csound,p->ifn)) != NULL) {
        p->elements = (int32_t) *p->ielements;
        p->offset = (int32_t) *p->idstoffset;
      }
      else return csoundInitError(csound, Str("vrandh: Invalid table."));
    ...
    if (p->auxch.auxp == NULL)
      csoundAuxAlloc(csound, p->elements * sizeof(MYFLT), &p->auxch);
...
static int32_t vrandi_set(CSOUND *csound,VRANDI *p)
{
    ...
    if (*p->iseed >= FL(0.0)) {                       /* new seed:*/
      ...
      else return csoundInitError(csound, Str("vrandi: Invalid table."));
    ...
    if (p->auxch.auxp == NULL) {
      csoundAuxAlloc(csound, p->elements * sizeof(MYFLT) * 3, &p->auxch);
    }
    elements = p->elements;
    num1 = (p->num1 = (MYFLT *) p->auxch.auxp);
...
static int32_t vecdly_set(CSOUND *csound, VECDEL *p)
{
    ...
    if (LIKELY((ftp = csoundFTnp2Find(csound,p->ifnOut)) != NULL)) {
      p->outvec = ftp->ftable;
      elements = (p->elements = (int32_t) *p->ielements);
      if (UNLIKELY( elements > (int32_t)ftp->flen ))
        return csoundInitError(csound,
    ...
    else return csoundInitError(csound, Str("vecdly: invalid output table"));
    if (LIKELY((ftp = csoundFTnp2Find(csound,p->ifnIn)) != NULL)) {
      p->invec = ftp->ftable;
      if (UNLIKELY(elements > (int32_t)ftp->flen))
        return csoundInitError(csound,
                                 Str("vecdelay: invalid num of elements"));
    }
    else return csoundInitError(csound, Str("vecdly: invalid input table"));
    if (LIKELY((ftp = csoundFTnp2Find(csound,p->ifnDel)) != NULL)) {
      p->dlyvec = ftp->ftable;
      if (UNLIKELY( elements > (int32_t)ftp->flen ))
        return csoundInitError(csound,
                                 Str("vecdelay: invalid num of elements"));
    }
    else return csoundInitError(csound, Str("vecdly: invalid delay table"));
    ...
    if (!*p->istod) {
      if (p->aux.auxp == NULL ||
        ...
        csoundAuxAlloc(csound, elements * sizeof(MYFLT *)
                 + n * elements * sizeof(MYFLT)
...
static int32_t vecdly(CSOUND *csound,VECDEL *p)
{
    ...
    if (UNLIKELY(buf==NULL)) {
      return csoundInitError(csound, Str("vecdly: not initialised"));
    }
    do {
      (*buf)[*indx] = *inVec++;
...
static int32_t vseg_set(CSOUND *csound,VSEG *p)
{
    ...
    if ((segp = (TSEG *) p->auxch.auxp) == NULL) {
      csoundAuxAlloc(csound, (int32)(nsegs+1)*sizeof(TSEG), &p->auxch);
      p->cursegp = segp = (TSEG *) p->auxch.auxp;
      (segp+nsegs)->cnt = MAXPOS;
    }
    argp = p->argums;
    if (UNLIKELY((nxtfunc = csoundFTnp2Find(csound,*argp++)) == NULL))
      return NOTOK;
    if ((ftp = csoundFTnp2Find(csound,p->ioutfunc)) != NULL) {
      p->vector = ftp->ftable;
      p->elements = (int32_t) *p->ielements;
    }
    else return NOTOK;
...
static int32_t vlinseg(CSOUND *csound,VSEG *p)
{
    ...
    if (UNLIKELY(p->auxch.auxp==NULL)) {
      return csoundInitError(csound, Str("tableseg: not initialised"));
    }
    segp = p->cursegp;
...
static int32_t vexpseg(CSOUND *csound,VSEG *p)
{
    ...
    if (UNLIKELY(p->auxch.auxp==NULL)) {
      return csoundInitError(csound, Str("tablexseg: not initialised"));
    }
    segp = p->cursegp;
    curtab = segp->function->ftable;
    nxttab = segp->nxtfunction->ftable;
...
static int32_t vphaseseg_set(CSOUND *csound,VPSEG *p)
{
    ...
    argp = p->argums;
    if ((nxtfunc = csoundFTnp2Finde(csound,*argp++)) == NULL)
      return NOTOK;
    if ((ftp = csoundFTnp2Finde(csound,p->ioutfunc)) != NULL) {
      p->vector = ftp->ftable;
      p->elements = (int32_t) *p->ielements;
    }
    ...
    do {
      ...
      curfunc = nxtfunc;
      dur = **argp++;
      if ((nxtfunc = csoundFTnp2Finde(csound,*argp++)) == NULL) return NOTOK;
      if (dur > FL(0.0)) {
        durtot+=dur;
        segp->d = dur; /* * CS_EKR; */
        segp->function = curfunc;
...
static int32_t kdel_set(CSOUND *csound,KDEL *p)
{
    ...
    if (!*p->istod) {
      if (p->aux.auxp == NULL || (uint32_t)(n*sizeof(MYFLT)) > p->aux.size)
        csoundAuxAlloc(csound, n * sizeof(MYFLT), &p->aux);
      else {
...
static int32_t kdelay(CSOUND *csound,KDEL *p)
{
    ...
    if (UNLIKELY(buf==NULL)) {
      return csoundInitError(csound, Str("vdelayk: not initialised"));
    }
...
static int32_t ca_set(CSOUND *csound,CELLA *p)
{
    ...
    if (LIKELY((ftp = csoundFTnp2Find(csound,p->ioutFunc)) != NULL)) {
      p->outVec = ftp->ftable;
      elements = (p->elements = (int32_t) *p->ielements);
      if (UNLIKELY( elements > (int32_t)ftp->flen ))
        return csoundInitError(csound, Str("cella: invalid num of elements"));
    }
    else return csoundInitError(csound, Str("cella: invalid output table"));
    if (LIKELY((ftp = csoundFTnp2Find(csound,p->initStateFunc)) != NULL)) {
    ...
    if (LIKELY((ftp = csoundFTnp2Find(csound,p->iRuleFunc)) != NULL)) {
      p->ruleVec = ftp->ftable;
    }
    else return csoundInitError(csound, Str("cella: invalid rule table"));
...
OENTRY vectorial_localops[] = {
  { "vtablei", S(MTABLEI),   TR, 1, "",   "iiiim", (SUBR)mtable_i,  NULL, NULL, NULL},
  { "vtablek", S(MTABLE),    TR, 3, "",   "kkkiz",
                                  (SUBR)mtable_set, (SUBR)mtable_k, NULL, NULL},
  { "vtablea", S(MTABLE),    TR, 3, "",   "akkiy",
                                  (SUBR)mtable_set, (SUBR)mtable_a, NULL, NULL},
  { "vtablewi", S(MTABLEIW), TB, 1, "",   "iiim", (SUBR)mtablew_i,  NULL, NULL, NULL},
  { "vtablewk", S(MTABLEW),  TB, 3, "",   "kkiz",
                                (SUBR)mtablew_set, (SUBR)mtablew_k, NULL, NULL},
  { "vtablewa", S(MTABLEW),  TB, 3, "",   "akiy",
                                (SUBR)mtablew_set, (SUBR)mtablew_a, NULL, NULL},
  { "vtabi", S(MTABI),       TR, 1, "",   "iim", (SUBR)mtab_i,  NULL, NULL, NULL},
  { "vtabk", S(MTAB),        TR, 3, "",   "kiz",
                                      (SUBR)mtab_set, (SUBR)mtab_k, NULL, NULL},
  { "vtaba", S(MTAB),        TR, 3, "",  "aiy",
                                      (SUBR)mtab_set, (SUBR)mtab_a, NULL, NULL},
  { "vtabwi", S(MTABIW),     TB, 1, "",  "iim", (SUBR)mtabw_i,  NULL, NULL, NULL},
  { "vtabwk", S(MTABW),      TB, 3, "",  "kiz",
                                       (SUBR)mtabw_set, (SUBR)mtabw_k, NULL, NULL},
  { "vtabwa", S(MTABW),      TB, 3, "",  "aiy",
                                       (SUBR)mtabw_set, (SUBR)mtabw_a, NULL, NULL},

  { "vadd",   S(VECTOROP),   TB, 3, "",  "ikkOO",
                                           (SUBR)vectorOp_set, (SUBR) vaddk, NULL, NULL},
  { "vadd_i", S(VECTOROPI),  TB, 1, "",  "iiio",  (SUBR) vadd_i, NULL, NULL, NULL},
  { "vmult",  S(VECTOROP),   TB, 3, "",  "ikkOO",
                                    (SUBR)vectorOp_set, (SUBR) vmultk, NULL, NULL},
  { "vmult_i", S(VECTOROPI), TB, 1, "",  "iiio", (SUBR) vmult_i, NULL, NULL, NULL},
  { "vpow",   S(VECTOROP),   TB, 3, "",  "ikkOO",
                                           (SUBR)vectorOp_set, (SUBR) vpowk, NULL, NULL},
  { "vpow_i", S(VECTOROPI),  TB, 1, "",  "iiio", (SUBR) vpow_i, NULL, NULL, NULL},
  { "vexp",   S(VECTOROP),   TB, 3, "",  "ikkOO",
                                           (SUBR)vectorOp_set, (SUBR) vexpk, NULL, NULL},
  { "vexp_i", S(VECTOROPI),  TB, 1, "",  "iiio", (SUBR) vexp_i, NULL, NULL, NULL},
  { "vaddv",  S(VECTORSOP),  TB, 3, "",  "iikOOO",
                                         (SUBR)vectorsOp_set, (SUBR) vaddvk, NULL, NULL},
  { "vaddv_i",  S(VECTORSOPI), TB, 1, "",  "iiioo", (SUBR)vaddv_i, NULL, NULL, NULL},
  { "vsubv",  S(VECTORSOP),  TB, 3, "",  "iikOOO",
                                         (SUBR)vectorsOp_set, (SUBR) vsubvk, NULL, NULL},
  { "vsubv_i",  S(VECTORSOPI),  TB, 1, "",  "iiioo",
                                           (SUBR)vsubv_i, NULL, NULL, NULL},
  { "vmultv", S(VECTORSOP),  TB, 3, "",  "iikOOO",
                                         (SUBR)vectorsOp_set, (SUBR) vmultvk, NULL, NULL},
  { "vmultv_i", S(VECTORSOPI),  TB, 1, "",  "iiioo", (SUBR)vmultv_i, NULL, NULL, NULL},
  { "vdivv",  S(VECTORSOP), TB,  3, "",  "iikOOO",
                                         (SUBR)vectorsOp_set, (SUBR) vdivvk, NULL, NULL},
  { "vdivv_i",  S(VECTORSOPI),  TB, 1, "",  "iiioo", (SUBR)vdivv_i, NULL, NULL, NULL},
  { "vpowv",  S(VECTORSOP),  TB, 3, "",  "iikOOO",
                                         (SUBR)vectorsOp_set, (SUBR) vpowvk, NULL, NULL},
  { "vpowv_i",  S(VECTORSOPI),  TB, 1, "",  "iiioo", (SUBR)vpowv_i, NULL, NULL, NULL},
  { "vexpv",  S(VECTORSOP),  TB, 3, "",  "iikOOO",
                                         (SUBR)vectorsOp_set, (SUBR) vexpvk, NULL, NULL},
  { "vexpv_i",  S(VECTORSOPI),  TB, 1, "",  "iiioo", (SUBR)vexpv_i, NULL, NULL, NULL},
  { "vcopy",  S(VECTORSOP),  TB, 3, "",  "iikOOO",
                                          (SUBR)vectorsOp_set, (SUBR) vcopy, NULL, NULL},
  { "vcopy_i", S(VECTORSOP), TB, 1, "",  "iiioo", (SUBR) vcopy_i, NULL, NULL, NULL},
  { "vmap",   S(VECTORSOPI), TB, 1, "",  "iiioo", (SUBR)vmap_i, NULL, NULL, NULL},
  { "vlimit", S(VLIMIT),  TR, 3, "",  "ikki",(SUBR)vlimit_set, (SUBR)vlimit, NULL, NULL},
  { "vwrap",  S(VLIMIT),  TB, 3, "",  "ikki",(SUBR)vlimit_set, (SUBR) vwrap, NULL, NULL},
  { "vmirror", S(VLIMIT),  TB,   3, "",  "ikki",(SUBR)vlimit_set, (SUBR)vmirror, NULL, NULL},
  { "vlinseg", S(VSEG),   TB, 3, "",  "iim", (SUBR)vseg_set,   (SUBR)vlinseg, NULL, NULL},
  { "vexpseg", S(VSEG),   TB, 3, "",  "iim", (SUBR)vseg_set, (SUBR)vexpseg, NULL, NULL},
  { "vrandh", S(VRANDH),  TB, 3, "",  "ikkiovoo",(SUBR)vrandh_set, (SUBR)vrandh, NULL, NULL},
  { "vrandi", S(VRANDI),  TB, 3, "",  "ikkiovoo",(SUBR)vrandi_set, (SUBR)vrandi, NULL, NULL},
  { "vport",  S(VPORT),   TB, 3, "",  "ikio",(SUBR)vport_set,  (SUBR)vport, NULL, NULL},
  { "vecdelay", S(VECDEL), TB, 3, "",  "iiiiio",(SUBR)vecdly_set, (SUBR)vecdly, NULL, NULL},
  { "vdelayk", S(KDEL),    0, 3, "k", "kkioo",(SUBR)kdel_set,  (SUBR)kdelay, NULL, NULL},
  { "vcella", S(CELLA),    TB, 3, "",  "kkiiiiip",(SUBR)ca_set, (SUBR)ca, NULL, NULL}
};

int32_t gab_vectorial_init_(CSOUND *csound)
{
    return
      csoundAppendOpcodes(csound, &(vectorial_localops[0]),
