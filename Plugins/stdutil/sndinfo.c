...
static int32_t sndinfo(CSOUND *csound, int32_t argc, char **argv)
{
    ...
    while (--argc) {
      ...
      fname = csoundFindInputFile(csound, infilnam, "SFDIR;SSDIR");
      if (UNLIKELY(fname == NULL)) {
        csoundMessage(csound, Str("%s:\n\tcould not find\n"), infilnam);
        retval = -1;
        continue;
      }
      ...
      if (UNLIKELY(hndl == NULL)) {
        csoundMessage(csound, Str("%s: Not a sound file\n"), fname);
        mfree(csound, fname);
        retval = -1;
        continue;
      }
      else {
        csoundNotifyFileOpened(csound, fname,
                            sftype2csfiletype(sflib_info.format), 0, 0);
        ...
        csoundMessage(csound,
                        ...
                        (long) (sfsampsize(sflib_info.format) * 8),
                        type2string(SF2TYPE(sflib_info.format)),
                        (MYFLT) sflib_info.frames / sflib_info.samplerate);
        csoundMessage(csound, Str("\t(%ld sample frames)\n"),
        ...
        if (instr_info) {
          ...
          if (sflib_command(hndl, SFC_GET_INSTRUMENT, &inst, sizeof (inst)) != 0) {
            csoundMessage(csound, Str("  Gain        : %d\n"),
                            inst.gain);
            csoundMessage(csound, Str("  Base note   : %d\n"),
                            inst.basenote);
            csoundMessage(csound, Str("  Velocity    : %d - %d\n"),
                            (int32_t) inst.velocity_lo, (int32_t) inst.velocity_hi);
            csoundMessage(csound, Str("  Key         : %d - %d\n"),
                            (int32_t) inst.key_lo, (int32_t) inst.key_hi);
            csoundMessage(csound, Str("  Loop points : %d\n"),
                            inst.loop_count);

            for (k = 0; k < inst.loop_count; k++)
              csoundMessage(csound, Str("  %-2d    Mode : %s    "
                                          "Start : %6d   End : %6d   "
                                          "Count : %6d\n"),
                              k,
        ...
        if (bcast_info) {
          ...
          if (sflib_command(hndl, SFC_GET_BROADCAST_INFO, &bext, sizeof (bext))
              != 0) {
            csoundMessage(csound, Str("Description      : %.*s\n"),
                            (int32_t) sizeof (bext.description), bext.description);
            csoundMessage(csound, Str("Originator       : %.*s\n"),
                            (int32_t) sizeof (bext.originator), bext.originator);
            csoundMessage(csound, Str("Origination ref  : %.*s\n"),
                            (int32_t) sizeof (bext.originator_reference),
                            bext.originator_reference);
            csoundMessage(csound, Str("Origination date : %.*s\n"),
                            (int32_t) sizeof (bext.origination_date),
                            bext.origination_date);
            csoundMessage(csound, Str("Origination time : %.*s\n"),
                            (int32_t) sizeof (bext.origination_time),
                            bext.origination_time);
            csoundMessage(csound, Str("BWF version      : %d\n"),
                            bext.version);
            csoundMessage(csound, Str("UMID             : %.*s\n"),
                            (int32_t) sizeof (bext.umid), bext.umid);
            csoundMessage(csound, Str("Coding history   : %.*s\n"),
...
{
    int32_t retval = csoundAddUtility(csound, "sndinfo", sndinfo);
    if (!retval) {
      retval =
        csoundSetUtilityDescription(csound, "sndinfo",
