...
#include "csoundCore_internal.h" /*                              UGENS6.C        */
#include "ugens6.h"
#include <math.h>
#include "auxfd.h"
...
int32_t downset(CSOUND *csound, DOWNSAMP *p)
{
    if (UNLIKELY((p->len = (uint32_t)*p->ilen) > CS_KSMPS))
      return csoundInitError(csound, "ilen > ksmps");
    return OK;
}
...
int32_t delset(CSOUND *csound, DELAY *p)
{
    ...
    if ((auxp = p->auxch.auxp) == NULL ||
      ...
      csoundAuxAlloc(csound, (int32_t)npts*sizeof(MYFLT), &p->auxch);
      auxp = p->auxch.auxp;
      p->npts = npts;
    }
...
int32_t delrset(CSOUND *csound, DELAYR *p)
{
    ...
    if (UNLIKELY(!IS_ASIG_ARG(p->ar)))
      return csoundInitError(csound, Str("delayr: invalid outarg type"));
...
static DELAYR *delayr_find(CSOUND *csound, MYFLT *ndx)
{
    ...
    if (UNLIKELY(d == NULL)) {
      csoundInitError(csound, Str("deltap: associated delayr not found"));
      return NULL;
    }
    if (!n)
...
int32_t cmbset(CSOUND *csound, COMB *p)
{
    ...
    if (p->auxch.auxp == NULL || (uint32_t)nbytes != p->auxch.size) {
      csoundAuxAlloc(csound, (int32_t)nbytes, &p->auxch);
...
int32_t rvbset(CSOUND *csound, REVERB *p)
{
    if (p->auxch.auxp == NULL) {                        /* if no space yet, */
      ...
      csoundAuxAlloc(csound, csound->revlpsum * sizeof(MYFLT), &p->auxch);
      ...
      if (UNLIKELY(p->adr6 + *sizp != (MYFLT *) p->auxch.endp)) {
        return csoundInitError(csound, Str("revlpsiz inconsistent\n"));
      }
      p->prvt = FL(0.0);
    }
...
static OENTRY ugens6_localops[] = {
    {"=.down", sizeof(DOWNSAMP), 0, 3, "k", "ao", (SUBR)downset, (SUBR)downsamp,
     NULL, NULL},
    {"=.up", sizeof(UPSAMP), 0, 2, "a", "k", NULL, (SUBR)upsamp, NULL, NULL},
    {"a.k", sizeof(INTERP), 0, 3, "a", "k", (SUBR)a_k_set, (SUBR)interp, NULL,
     NULL},
    {"alpass", sizeof(COMB), 0, 3, "a", "axioo", (SUBR)cmbset, (SUBR)alpass,
     NULL, NULL},
    {"comb", sizeof(COMB), 0, 3, "a", "akioo", (SUBR)cmbset, (SUBR)comb, NULL,
     NULL},
    {"combinv", sizeof(COMB), 0, 3, "a", "akioo", (SUBR)cmbset, (SUBR)invcomb,
     NULL, NULL},
    {"delay1", sizeof(DELAY1), 0, 3, "a", "ao", (SUBR)del1set, (SUBR)delay1,
     NULL, NULL},
    {"delay", sizeof(DELAY), 0, 3, "a", "aio", (SUBR)delset, (SUBR)delay, NULL,
     NULL},
    {"delayr", sizeof(DELAYR), 0, 3, "aX", "io", (SUBR)delrset, (SUBR)delayr,
     NULL, NULL},
    {"delayw", sizeof(DELAYW), 0, 3, "", "a", (SUBR)delwset, (SUBR)delayw, NULL,
     NULL},
    {"deltap3", sizeof(DELTAP), 0, 3, "a", "xo", (SUBR)tapset, (SUBR)deltap3,
     NULL, NULL},
    {"deltap", sizeof(DELTAP), 0, 3, "a", "ko", (SUBR)tapset, (SUBR)deltap,
     NULL, NULL},
    {"deltapi", sizeof(DELTAP), 0, 3, "a", "xo", (SUBR)tapset, (SUBR)deltapi,
     NULL, NULL},
    {"deltapn", sizeof(DELTAP), 0, 3, "a", "xo", (SUBR)tapset, (SUBR)deltapn,
     NULL, NULL},
    {"deltapx", sizeof(DELTAPX), 0, 3, "a", "aio", (SUBR)tapxset, (SUBR)deltapx,
     NULL, NULL},
    {"deltapxw", sizeof(DELTAPX), 0, 3, "", "aaio", (SUBR)tapxset,
     (SUBR)deltapxw, NULL, NULL},
    {"diff", sizeof(INDIFF), 0, 3, "a", "xo", (SUBR)indfset, (SUBR)diff, NULL,
     NULL},
    {"diff.k", sizeof(INDIFF), 0, 3, "k", "xo", (SUBR)indfset, (SUBR)kdiff,
     NULL, NULL},
    {"downsamp", sizeof(DOWNSAMP), 0, 3, "k", "ao", (SUBR)downset,
     (SUBR)downsamp, NULL, NULL},
    {"integ", sizeof(INDIFF), 0, 3, "a", "xo", (SUBR)indfset, (SUBR)integrate,
     NULL, NULL},
    {"integ.k", sizeof(INDIFF), 0, 3, "k", "xo", (SUBR)indfset, (SUBR)kntegrate,
     NULL, NULL},
    {"interp", sizeof(INTERP), 0, 3, "a", "kooo", (SUBR)interpset, (SUBR)interp,
     NULL, NULL},
    {"k.a", sizeof(DOWNSAMP), 0, 3, "k", "ao", (SUBR)downset, (SUBR)downsamp,
     NULL, NULL},
    {"pan", sizeof(PAN), 0, 3, "aaaa", "akkioo", (SUBR)panset, (SUBR)pan, NULL,
     NULL},
    {"reverb", sizeof(REVERB), 0, 3, "a", "ako", (SUBR)rvbset, (SUBR)reverb,
     NULL, NULL},
    {"samphold", sizeof(SAMPHOLD), 0, 3, "a", "xxoo", (SUBR)samphset,
     (SUBR)samphold, NULL, NULL},
    {"samphold.k", sizeof(SAMPHOLD), 0, 3, "k", "xxoo", (SUBR)samphset,
     (SUBR)ksmphold, NULL, NULL},
    {"upsamp", sizeof(UPSAMP), 0, 2, "a", "k", NULL, (SUBR)upsamp, NULL, NULL},
};

LINKAGE_BUILTIN(ugens6_localops)