...
int32_t s_opcode(CSOUND *csound, STRGET_OP *p){
  if (p->r->size < DEFAULT_STRING_SIZE){
    p->r->data = (char *) mrealloc(csound, p->r->data, DEFAULT_STRING_SIZE);
...
static void str_set(CSOUND *csound, int32_t ndx, const char *s)
{
  if (UNLIKELY(csound->strsets == NULL)) {
    ...
    csound->strsets = (char **) mcalloc(csound, (csound->strsmax + 1)
					       * sizeof(char*));
  }
  if (UNLIKELY(ndx > (int32_t) csound->strsmax)) {
    ...
    csound->strsets = (char**) mrealloc(csound, csound->strsets,
  ...
  if (UNLIKELY(ndx < 0))  {  /* -ve index */
    csoundInitError(csound, Str("illegal strset index"));
    return;
  }
  ...
  if (csound->strsets[ndx] != NULL) {
    ...
    if (UNLIKELY(csound->oparms->msglevel & CS_WARNMSG)) {
      csoundWarning(csound, Str("strset index conflict at %d"), ndx);
      csoundWarning(csound, Str("previous value: '%s', replaced with '%s'"),
		      csound->strsets[ndx], s);
    }
    mfree(csound, csound->strsets[ndx]);
  }
  csound->strsets[ndx] = (char*) mmalloc(csound, strlen(s) + 1);
  strcpy(csound->strsets[ndx], s);
  if ((csound->oparms->msglevel & 7) == 7)
    csoundMessage(csound, "Strsets[%d]: '%s'\n", ndx, s);
}
...
void strset_option(CSOUND *csound, char *s)
{
  ...
  if (UNLIKELY(!isdigit(*s))) {
    csoundWarning(csound, Str("--strset: invalid format"));
    return;
  }
  do {
...
static CS_NOINLINE int32_t StrOp_ErrMsg(void *p, const char *msg)
{
  ...
  if (UNLIKELY(csound->ids != NULL && csound->ids->insdshead == csound->curip))
    return csoundInitError(csound, "%s: %s", opname, Str(msg));
  else if (UNLIKELY(((OPDS*) p)->insdshead->pds != NULL))
    return csoundPerfError(csound, (OPDS*)p,
			     "%s: %s", opname, Str(msg));
  else
    csoundWarning(csound, "%s: %s", opname, Str(msg));
...
int32_t strassign_k(CSOUND *csound, STRCPY_OP *p) {
  if(p->r != p->str) {
  if((uint64_t)p->str->timestamp == csoundGetKcounter(csound)) {
  CS_TYPE *strType = csoundGetTypeForArg(p->str);    
...
int32_t strcpy_opcode_p(CSOUND *csound, STRGET_OP *p)
{
  if (isstrcod(*p->indx)) {
   ...
   if (strlen(ss) >= p->r->size) {
    ...
    mfree(csound, p->r->data);
    p->r->data = strarg2name(csound, NULL, p->indx, "soundin.", 0);
...
int32_t strcat_opcode(CSOUND *csound, STRCAT_OP *p)
{
  ...
  if(p->str1 != p->r && p->str2 != p->r) {
    ...
    return OK;
     ...
     if(size >= p->r->size) {
       p->r->data =
 	mrealloc(csound, p->r->data, 2*size);
       p->r->size = 2*size;
    }      
		...
		int32_t strCode)      /* bit mask for string arguments   */
{
  ...
  while (1) {
    if (UNLIKELY(i >= siz)) {
      ...
      siz *= 2;
      strseg = mrealloc(csound, strseg, siz);
    }
    if (*fmt != '%' && *fmt != '\0') {
      strseg[i++] = *fmt++;
      continue;
	...
	if (strlen(strseg) + 24 > (uint32_t)maxChars) {
	  int32_t offs = outstring - str->data;
	  str->data = mrealloc(csound, str->data, str->size  + 13);
	  if(str->data == NULL) {
	    return StrOp_ErrMsg(p, Str("memory allocation failure"));
	  }
	  str->size += 24;
...
static CS_NOINLINE int32_t printf_opcode_(CSOUND *csound, PRINTF_OP *p)
{
  ...
  if (LIKELY(err == OK))
    csoundMessageS(csound, CSOUNDMSG_ORCH, "%s", buf.data);
...
int32_t puts_opcode_init(CSOUND *csound, PUTS_OP *p)
{
    if (*p->ktrig > FL(0.0)) {
        if (!p->noNewLine)
          ...
          csoundMessageS(csound, CSOUNDMSG_ORCH, "%s", (char*) p->str->data);
    }
    p->prv_ktrig = *p->ktrig;
...
int32_t strsub_opcode(CSOUND *csound, STRSUB_OP *p)
{
  int64_t kcnt = csoundGetKcounter(csound);
    ...
    if (p->Sdst->data == NULL || p->Sdst->size < p->Ssrc->size) {
      ...
      if (p->Sdst->data != NULL) mfree(csound, p->Sdst->data);
...
int32_t getcfg_opcode(CSOUND *csound, GETCFG_OP *p)
{
  ...
  if (p->Sdst->size < 32){
    mfree(csound, p->Sdst->data);
    p->Sdst->data = mcalloc(csound,32);
    p->Sdst->size = 32;
  }
  ...
  case 7:             /* is the channel I/O callback set ? (0: no, 1: yes) */
    ...
    break;
  default:
    return csoundInitError(csound, Str("invalid option code: %g"),
			     *(p->iopt));
  }
  if (s != NULL) {
...
int32_t print_type_opcode(CSOUND* csound, PRINT_TYPE_OP* p) {
  ...
  csoundMessage(csound, "Variable Type: %s\n", varType->varTypeName);

  return OK;
}

static OENTRY str_ops_localops[] = {
    {"#=.S", sizeof(STRCPY_OP), 0, 3, "S", "S", (SUBR)strcpy_opcode_S,
     (SUBR)strassign_k, (SUBR)NULL, NULL},
    {"=.S", sizeof(STRCPY_OP), 0, 3, "S", "S", (SUBR)strcpy_opcode_S,
     (SUBR)strassign_k, (SUBR)NULL, NULL},
    {"=.T", sizeof(STRCPY_OP), 0, 1, "S", "i", (SUBR)strcpy_opcode_p,
     (SUBR)NULL, (SUBR)NULL, NULL},
    {"changed2.S", sizeof(STRCHGD), 0, 3, "k", "S", (SUBR)str_changed,
     (SUBR)str_changed_k, NULL, NULL},
    {"changed.S", sizeof(STRCHGD), 0, 3, "k", "S", (SUBR)str_changed,
     (SUBR)str_changed_k, NULL, NULL},
    {"getcfg", sizeof(GETCFG_OP), 0, 1, "S", "i", (SUBR)getcfg_opcode, NULL,
     NULL, NULL},
    {"print_type", sizeof(PRINT_TYPE_OP), 0, 1, "", ".",
     (SUBR)print_type_opcode, NULL, NULL, NULL},
    {"printf", sizeof(PRINTF_OP), WR, 3, "", "SkN", (SUBR)printf_opcode_set,
     (SUBR)printf_opcode_perf, NULL, NULL},
    {"printf_i", sizeof(PRINTF_OP), 0, 1, "", "SiN", (SUBR)printf_opcode_init,
     NULL, NULL, NULL},
    {"puts", sizeof(PUTS_OP), WR, 3, "", "Sko", (SUBR)puts_opcode_init,
     (SUBR)puts_opcode_perf, NULL, NULL},
    {"S", sizeof(STRGET_OP), 0, 1, "S", "i", (SUBR)s_opcode, NULL, NULL, NULL},
    {"S", sizeof(STRGET_OP), 0, 3, "S", "k", (SUBR)s_opcode, (SUBR)s_opcode_k,
     NULL, NULL},
    {"sprintf", sizeof(SPRINTF_OP), 0, 1, "S", "STN", (SUBR)sprintf_opcode,
     NULL, NULL, NULL},
    {"sprintfk", sizeof(SPRINTF_OP), WR, 3, "S", "SUN", (SUBR)sprintf_opcode,
     (SUBR)sprintf_opcode, NULL, NULL},
    {"strcat", sizeof(STRCAT_OP), 0, 1, "S", "SS", (SUBR)strcat_opcode, NULL,
     NULL, NULL},
    {"strcatk", sizeof(STRCAT_OP), 0, 3, "S", "SS", (SUBR)strcat_opcode,
     (SUBR)strcat_opcode, NULL, NULL},
    {"strchar", sizeof(STRCHAR_OP), 0, 1, "i", "So", (SUBR)strchar_opcode, NULL,
     NULL, NULL},
    {"strchark", sizeof(STRCHAR_OP), 0, 3, "k", "SO", (SUBR)strchar_opcode,
     (SUBR)strchar_opcode, NULL, NULL},
    {"strcmp", sizeof(STRCMP_OP), 0, 1, "i", "SS", (SUBR)strcmp_opcode, NULL,
     NULL, NULL},
    {"strcmpk", sizeof(STRCMP_OP), 0, 3, "k", "SS", (SUBR)strcmp_opcode,
     (SUBR)strcmp_opcode, NULL, NULL},
    {"strcpy", sizeof(STRCPY_OP), 0, 1, "S", "S", (SUBR)strcpy_opcode_S, NULL,
     NULL, NULL},
    {"strcpy", sizeof(STRGET_OP), 0, 1, "S", "i", (SUBR)strcpy_opcode_p, NULL,
     NULL, NULL},
    {"strcpyk", sizeof(STRCPY_OP), 0, 3, "S", "S", (SUBR)strcpy_opcode_S,
     (SUBR)strcpy_opcode_S, NULL, NULL},
    {"strcpyk.k", sizeof(STRGET_OP), 0, 3, "S", "k", (SUBR)strcpy_opcode_p,
     (SUBR)strcpy_opcode_p, NULL, NULL},
    {"strindex", sizeof(STRINDEX_OP), 0, 1, "i", "SS", (SUBR)strindex_opcode,
     NULL, NULL, NULL},
    {"strindexk", sizeof(STRINDEX_OP), 0, 3, "k", "SS", (SUBR)strindex_opcode,
     (SUBR)strindex_opcode, NULL, NULL},
    {"strlen", sizeof(STRLEN_OP), 0, 1, "i", "S", (SUBR)strlen_opcode, NULL,
     NULL, NULL},
    {"strlenk", sizeof(STRLEN_OP), 0, 3, "k", "S", (SUBR)strlen_opcode,
     (SUBR)strlen_opcode, NULL, NULL},
    {"strlower", sizeof(STRUPPER_OP), 0, 1, "S", "S", (SUBR)strlower_opcode,
     NULL, NULL, NULL},
    {"strlowerk", sizeof(STRUPPER_OP), 0, 3, "S", "S", (SUBR)strlower_opcode,
     (SUBR)strlower_opcode, NULL, NULL},
    {"strrindex", sizeof(STRINDEX_OP), 0, 1, "i", "SS", (SUBR)strrindex_opcode,
     NULL, NULL, NULL},
    {"strrindexk", sizeof(STRINDEX_OP), 0, 3, "k", "SS", (SUBR)strrindex_opcode,
     (SUBR)strrindex_opcode, NULL, NULL},
    {"strsub", sizeof(STRSUB_OP), 0, 1, "S", "Soj", (SUBR)strsub_opcode, NULL,
     NULL, NULL},
    {"strsubk", sizeof(STRSUB_OP), 0, 3, "S", "Skk", (SUBR)strsub_opcode,
     (SUBR)strsub_opcode, NULL, NULL},
    {"strtod", sizeof(STRSET_OP), 0, 1, "i", "S", (SUBR)strtod_opcode_S, NULL,
     NULL, NULL},
    {"strtod", sizeof(STRTOD_OP), 0, 1, "i", "i", (SUBR)strtod_opcode_p, NULL,
     NULL, NULL},
    {"strtodk", sizeof(STRSET_OP), 0, 3, "k", "S", (SUBR)strtod_opcode_S,
     (SUBR)strtod_opcode_S, NULL, NULL},
    {"strtol", sizeof(STRSET_OP), 0, 1, "i", "S", (SUBR)strtol_opcode_S, NULL,
     NULL, NULL},
    {"strtol", sizeof(STRTOD_OP), 0, 1, "i", "i", (SUBR)strtol_opcode_p, NULL,
     NULL, NULL},
    {"strtolk", sizeof(STRSET_OP), 0, 3, "k", "S", (SUBR)strtol_opcode_S,
     (SUBR)strtol_opcode_S, NULL},
    {"strupper", sizeof(STRUPPER_OP), 0, 1, "S", "S", (SUBR)strupper_opcode,
     NULL, NULL, NULL},
    {"strupperk", sizeof(STRUPPER_OP), 0, 3, "S", "S", (SUBR)strupper_opcode,
     (SUBR)strupper_opcode, NULL, NULL},
    {"init.S", sizeof(STRCPY_OP), 0, 1, "S", "S", (SUBR)strcpy_opcode_S, NULL,
     NULL, NULL},
    {"init.Si", sizeof(STRCPY_OP), 0, 1, "S", "i", (SUBR)strcpy_opcode_p, NULL,
     NULL, NULL},
    {"strget", sizeof(STRGET_OP), 0, 1, "S", "i", (SUBR)strget_init, NULL, NULL,
     NULL},
    {"strset", sizeof(STRSET_OP), 0, 1, "", "iS", (SUBR)strset_init, NULL, NULL,
     NULL},
#ifdef HAVE_CURL
    {"strfromurl", sizeof(STRCPY_OP), 0, 1, "S", "S", (SUBR)str_from_url},
#endif
};

LINKAGE_BUILTIN(str_ops_localops)