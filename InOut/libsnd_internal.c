...
static void writesf(CSOUND *csound, const MYFLT *outbuf, int nbytes)
{
    ...
    if (UNLIKELY(STA_LIBSND(outfile) == NULL))
      return;
    n = (int) sflib_write_MYFLT(STA_LIBSND(outfile), (MYFLT*) outbuf,
                             nbytes / sizeof(MYFLT)) * (int) sizeof(MYFLT);
    if (UNLIKELY(n < nbytes))
      sndwrterr(csound, n, nbytes);
    if (UNLIKELY(O->rewrt_hdr))
      rewriteheader((void *)STA_LIBSND(outfile));
    switch (O->heartbeat) {
      case 1:
...
static void writesf_dither_16(CSOUND *csound, const MYFLT *outbuf, int nbytes)
{
    ...
    STA_LIBSND(dither) = dith;
    n = (int) sflib_write_MYFLT(STA_LIBSND(outfile), (MYFLT*) outbuf,
                             nbytes / sizeof(MYFLT)) * (int) sizeof(MYFLT);
    if (UNLIKELY(n < nbytes))
      sndwrterr(csound, n, nbytes);
    if (UNLIKELY(O->rewrt_hdr))
      rewriteheader(STA_LIBSND(outfile));
    switch (O->heartbeat) {
      case 1:
        csoundMessageS(csound, CSOUNDMSG_REALTIME,
                                 "%c\010", "|/-\\"[csound->nrecs & 3]);
...
static int readsf(CSOUND *csound, MYFLT *inbuf, int inbufsize)
{
    ...
    i = (int) sflib_read_MYFLT(STA_LIBSND(infile), inbuf, n);
    if (UNLIKELY(i < 0))
      return inbufsize;
...
void sfopenin(CSOUND *csound)           /* init for continuous soundin */
{
    ...
    STA_LIBSND(inbufrem) = (uint32) 0;    /* start with empty buffer */
    sfname = O->infilename;
    if (UNLIKELY(sfname == NULL || sfname[0] == '\0'))
      csoundDie(csound, Str("error: no input file name"));

    if (strcmp(sfname, "stdin") == 0) {
      STA_LIBSND(pipdevin) = 1;
    }
#ifdef PIPES
    else if (sfname[0] == '|') {
      STA_LIBSND(pin) = _popen(sfname + 1, "r");
      isfd = fileno(STA_LIBSND(pin));
      STA_LIBSND(pipdevin) = 1;
    }
#endif
    else {
      ...
      if (parm.devNum >= 0) {
        ...
        STA_LIBSND(pipdevin) = 2;       /* no backward seeks !     */
        goto inset;             /* no header processing    */
      }
    }
    /* open file */
    memset(&sfinfo, 0, sizeof(SFLIB_INFO));
    if (STA_LIBSND(pipdevin)) {
      STA_LIBSND(infile) = sflib_open_fd(isfd, SFM_READ, &sfinfo, 0);
      if (UNLIKELY(STA_LIBSND(infile) == NULL)) {
      ...
      STA_LIBSND(infile) = sflib_open(fullName, SFM_READ, &sfinfo);
      if (STA_LIBSND(infile) == NULL) {
        ...
        STA_LIBSND(infile) = sflib_open(fullName, SFM_READ, &sfinfo);  /* try again */
      }
      if (UNLIKELY(STA_LIBSND(infile) == NULL)) {
        const char *sfError = Str(sflib_strerror(NULL));
 ...
 inset:
    /* calc inbufsize reqd */
    STA_LIBSND(inbufsiz) = (unsigned) (O->inbufsamps * sizeof(MYFLT));
    STA_LIBSND(inbuf) = (MYFLT*) mcalloc(csound,
                                         STA_LIBSND(inbufsiz)); /* alloc inbuf space */
    if (STA_LIBSND(pipdevout) == 2) {
...
{                                               /* (not called if nosound)  */
    ...
    STA_LIBSND(sfoutname) = fName = O->outfilename;

    if (strcmp(fName, "stdout") == 0) {
      STA_LIBSND(pipdevout) = 1;
    }
#ifdef PIPES
    else if (fName[0] == '|') {
      STA_LIBSND(pout) = _popen(fName+1, "w");
      osfd = fileno(STA_LIBSND(pout));
      STA_LIBSND(pipdevout) = 1;
      ...
      if (parm.devNum >= 0) {
        ...
        STA_LIBSND(outbufrem)  = parm.bufSamp_SW * parm.nChannels;
        STA_LIBSND(pipdevout)  = 2;      /* no backward seeks !   */
    ...
    /* open file */
    if (STA_LIBSND(pipdevout)) {
      STA_LIBSND(outfile) = sflib_open_fd(osfd, SFM_WRITE, &sfinfo, 0);
#ifdef PIPES
      if (STA_LIBSND(outfile) == NULL) {
        char fmt_name[6];
        if (O->sfsampsize == 8) {
      ...
      if (UNLIKELY(STA_LIBSND(outfile) == NULL))
        csoundDie(csound, Str("sfinit: cannot open fd %d\n%s"), osfd,
                  Str(sflib_strerror(NULL)));
      sflib_command(STA_LIBSND(outfile), SFC_SET_VBR_ENCODING_QUALITY,
      ...
      STA_LIBSND(sfoutname) = fullName;
      STA_LIBSND(outfile)   = sflib_open(fullName, SFM_WRITE, &sfinfo);
      if (UNLIKELY(STA_LIBSND(outfile) == NULL))
    ...
    if(O->filetyp == TYP_MPEG) {
      ...
      sf_command(STA_LIBSND(outfile), SFC_SET_BITRATE_MODE,
    ...
    if (O->outformat != AE_FLOAT && O->outformat != AE_DOUBLE)
      sflib_command(STA_LIBSND(outfile), SFC_SET_CLIPPING, NULL, SFLIB_TRUE);
    sflib_command(STA_LIBSND(outfile), SFC_SET_ADD_PEAK_CHUNK,
    ...
    if (csound->dither_output) {        /* This may not be written yet!! */
      ...
      sflib_command(STA_LIBSND(outfile), SFC_SET_DITHER_ON_WRITE,
    ...
    if ((s = csound->SF_id_title) != NULL && *s != '\0')
      sflib_set_string(STA_LIBSND(outfile), SF_STR_TITLE, s);
    if ((s = csound->SF_csd_licence) == NULL || *s == '\0')
      s = csound->SF_id_copyright;
    if (s != NULL && *s != '\0')
      sflib_set_string(STA_LIBSND(outfile), SF_STR_COPYRIGHT, s);
      ...
      buff[255] = '\0';
      sflib_set_string(STA_LIBSND(outfile), SF_STR_COPYRIGHT, buff);
    }
    if ((s = csound->SF_id_software) != NULL && *s != '\0')
      sflib_set_string(STA_LIBSND(outfile), SF_STR_SOFTWARE, s);
    if ((s = csound->SF_id_artist) != NULL && *s != '\0')
      sflib_set_string(STA_LIBSND(outfile), SF_STR_ARTIST, s);
    if ((s = csound->SF_id_comment) != NULL && *s != '\0')
      sflib_set_string(STA_LIBSND(outfile), SF_STR_COMMENT, s);
    if ((s = csound->SF_id_date) != NULL && *s != '\0')
      sflib_set_string(STA_LIBSND(outfile), SF_STR_DATE, s);
    /* file is now open */
    STA_LIBSND(osfopen) = 1;


 outset:
    O->sfsampsize = (int) sfsampsize(FORMAT2SF(O->outformat));
    /* calc outbuf size & alloc bufspace */
    STA_LIBSND(outbufsiz) = O->outbufsamps * sizeof(MYFLT);
    STA_LIBSND(outbufp)   = STA_LIBSND(outbuf) = mmalloc(csound, STA_LIBSND(outbufsiz));
    if (STA_LIBSND(pipdevout) == 2) {
      csoundMessage(csound,
                      Str("writing %d sample blks of %lu-bit floats to %s\n"),
                      O->outbufsamps, (unsigned long) sizeof(MYFLT)*8,
                      STA_LIBSND(sfoutname));

    }
    else {
      csoundMessage(csound, Str("writing %d-byte blks of %s to %s"),
                    O->outbufsamps * O->sfsampsize,
                    getstrformat(O->outformat), STA_LIBSND(sfoutname));
    ...
    STA_LIBSND(osfopen)   = 1;
    STA_LIBSND(outbufrem) = O->outbufsamps;
}

void sfclosein(CSOUND *csound)
{
    alloc_globals(csound);
    if (!STA_LIBSND(isfopen))
      return;
    if (STA_LIBSND(pipdevin) == 2 && (!STA_LIBSND(osfopen) || STA_LIBSND(pipdevout) != 2)) {
      /* close only if not open for output too */
      csound->rtclose_callback(csound);
    }
    else if (STA_LIBSND(pipdevin) != 2) {
      if (STA_LIBSND(infile) != NULL)
        sflib_close(STA_LIBSND(infile));
#ifdef PIPES
      if (STA_LIBSND(pin) != NULL) {
        _pclose(STA_LIBSND(pin));
        STA_LIBSND(pin) = NULL;
      }
#endif
      STA_LIBSND(infile) = NULL;
    }
    STA_LIBSND(isfopen) = 0;
}
...
void sfcloseout(CSOUND *csound)
{
    ...
    alloc_globals(csound);
    if (!STA_LIBSND(osfopen))
      return;
    if ((nb = (O->outbufsamps - STA_LIBSND(outbufrem)) * sizeof(MYFLT)) > 0) {
      /* flush outbuffer */
      csound->nrecs++;
      csound->audtran(csound, STA_LIBSND(outbuf), nb);
    }
    if (STA_LIBSND(pipdevout) == 2 && (!STA_LIBSND(isfopen) || STA_LIBSND(pipdevin) != 2)) {
    ...
    if (STA_LIBSND(pipdevout) == 2)
      goto report;
    if (STA_LIBSND(outfile) != NULL) {
      if (!STA_LIBSND(pipdevout) && O->outformat != AE_VORBIS)
        sflib_command(STA_LIBSND(outfile), SFC_UPDATE_HEADER_NOW, NULL, 0);
      sflib_close(STA_LIBSND(outfile));
      STA_LIBSND(outfile) = NULL;
    }
#ifdef PIPES
    if (STA_LIBSND(pout) != NULL) {
      _pclose(STA_LIBSND(pout));
      STA_LIBSND(pout) = NULL;
    }
#endif
...
static void sndwrterr(CSOUND *csound, int nret, int nput)
{
    ...
    STA_LIBSND(outbufrem) = csound->oparms->outbufsamps;  /* consider buf is flushed */
...
void sfnopenout(CSOUND *csound)
{
    ...
    STA_LIBSND(outbufrem) = csound->oparms->outbufsamps;
}
...
static inline void sndfilein_(CSOUND *csound, MYFLT scaleFac)
{
    ...
    nsmps = csound->nspin;
    bufpos = (int) O->inbufsamps - (int) STA_LIBSND(inbufrem);
    for (i = 0; i<nsmps; i++) {
      if ((int) STA_LIBSND(inbufrem) < 1) {
        STA_LIBSND(inbufrem) = 0U;
        do {
          n = ((int) O->inbufsamps - (int) STA_LIBSND(inbufrem)) * (int) sizeof(MYFLT);
          n = csound->audrecv(csound, STA_LIBSND(inbuf) + (int) STA_LIBSND(inbufrem), n);
          STA_LIBSND(inbufrem) += (unsigned int) (n / (int) sizeof(MYFLT));
        } while ((int) STA_LIBSND(inbufrem) < (int) O->inbufsamps);
        bufpos = 0;
      }
      csound->spin[i] = STA_LIBSND(inbuf)[bufpos++] * scaleFac;
      STA_LIBSND(inbufrem)--;
    }
}
...
void iotranset(CSOUND *csound)
{
    ...
    STA_LIBSND(inbufrem)   = (unsigned int) O->inbufsamps;
    STA_LIBSND(outbufrem)  = (unsigned int) O->outbufsamps;
    ...
    STA_LIBSND(inbufsiz)  = (unsigned int) (O->inbufsamps * (int) sizeof(MYFLT));
    STA_LIBSND(inbuf)     = (MYFLT*) mcalloc(csound, STA_LIBSND(inbufsiz));
    STA_LIBSND(outbufsiz) = (unsigned int) (O->outbufsamps * (int) sizeof(MYFLT));
    STA_LIBSND(outbuf)    = (MYFLT*) mcalloc(csound, STA_LIBSND(outbufsiz));
    STA_LIBSND(outbufp)   = STA_LIBSND(outbuf);
    O->sfread      = 1;
    O->sfwrite     = 1;
    STA_LIBSND(osfopen)   = 1;
}
