...
int32_t tabler_init(CSOUND *csound, TABL *p) {
    ...
    if (UNLIKELY((p->ftp = csoundFTnp2Find(csound, p->ftable)) == NULL))
...
int32_t table_length(CSOUND *csound, TLEN *p) {
    FUNC *ftp;
    if (UNLIKELY((ftp = csoundFTnp2Find(csound, p->ftable)) == NULL)) {
      csoundWarning(csound, Str("table: could not find ftable %d"),
                      (int32_t) *p->ftable);
      *p->ans = FL(-1.0);
      return NOTOK;
    }
...
int32_t table_copy(CSOUND *csound, TGP *p) {
    ...
    if (UNLIKELY((dest = csoundFTnp2Find(csound, p->ftable)) == NULL ||
                 (src = csoundFTnp2Find(csound, p->ftsrc)) == NULL)) {
      csoundWarning(csound,
...
int32_t table_mix(CSOUND *csound, TABLMIX *p) {
    ...
    if (UNLIKELY((ftp = csoundFTnp2Find(csound, p->tab)) == NULL)) {
      csoundWarning(csound,
    ...
    if (UNLIKELY((ftp1 = csoundFTnp2Find(csound, p->tab1)) == NULL)) {
      csoundWarning(csound,
    ...
    if (UNLIKELY((ftp2 = csoundFTnp2Find(csound, p->tab2)) == NULL)) {
      csoundWarning(csound,
...
static OENTRY ugtabs_localops[] = {
    {"ptable3.a", sizeof(TABLE), TR | _QQ, 3, "a", "xiooo", (SUBR)tabl_setup,
     (SUBR)table3r_audio, NULL, NULL},
    {"ptable3.i", sizeof(TABLE), TR | _QQ, 1, "i", "iiooo", (SUBR)table3r_init,
     NULL, NULL, NULL},
    {"ptable3.k", sizeof(TABLE), TR | _QQ, 3, "k", "xiooo", (SUBR)tabl_setup,
     (SUBR)table3r_kontrol, NULL, NULL},
    {"ptable.a", sizeof(TABLE), TR | _QQ, 3, "a", "xiooo", (SUBR)tabl_setup,
     (SUBR)tabler_audio, NULL, NULL},
    {"ptable.i", sizeof(TABLE), TR | _QQ, 1, "i", "iiooo", (SUBR)tabler_init,
     NULL, NULL, NULL},
    {"ptable.k", sizeof(TABLE), TR | _QQ, 3, "k", "xiooo", (SUBR)tabl_setup,
     (SUBR)tabler_kontrol, NULL, NULL},
    {"ptablei.a", sizeof(TABLE), TR | _QQ, 3, "a", "xiooo", (SUBR)tabl_setup,
     (SUBR)tableir_audio, NULL, NULL},
    {"ptablei.i", sizeof(TABLE), TR | _QQ, 1, "i", "iiooo", (SUBR)tableir_init,
     NULL, NULL, NULL},
    {"ptablei.k", sizeof(TABLE), TR | _QQ, 3, "k", "xiooo", (SUBR)tabl_setup,
     (SUBR)tableir_kontrol, NULL, NULL},
    {"ptableiw", sizeof(TABLEW), TW | _QQ, 1, "", "iiiooo", (SUBR)tablew_init,
     NULL, NULL, NULL},
    {"ptablew.aa", sizeof(TABLEW), TB, 3, "", "aaiooo", (SUBR)tabl_setup,
     (SUBR)tablew_audio, NULL, NULL},
    {"ptablew.kk", sizeof(TABLEW), TB, 3, "", "kkiooo", (SUBR)tabl_setup,
     (SUBR)tablew_kontrol, NULL, NULL},
    {"table3.a", sizeof(TABL), TR, 3, "a", "xiooo", (SUBR)tabl_setup,
     (SUBR)table3r_audio, NULL, NULL},
    {"table3.i", sizeof(TABL), TR, 1, "i", "iiooo", (SUBR)table3r_init, NULL,
     NULL, NULL},
    {"table3.k", sizeof(TABL), TR, 3, "k", "xiooo", (SUBR)tabl_setup,
     (SUBR)table3r_kontrol, NULL, NULL},
    {"table3kt", sizeof(TABL), TR, 3, "k", "xkooo", (SUBR)tablkt_setup,
     (SUBR)table3rkt_kontrol, NULL, NULL},
    {"table3kt.a", sizeof(TABL), TR, 3, "a", "xkooo", (SUBR)tablkt_setup,
     (SUBR)table3rkt_audio, NULL, NULL},
    {"table.a", sizeof(TABL), TR, 3, "a", "xiooo", (SUBR)tabl_setup,
     (SUBR)tabler_audio, NULL, NULL},
    {"table.i", sizeof(TABL), TR, 1, "i", "iiooo", (SUBR)tabler_init, NULL,
     NULL, NULL},
    {"table.k", sizeof(TABL), TR, 3, "k", "xiooo", (SUBR)tabl_setup,
     (SUBR)tabler_kontrol, NULL, NULL},
    {"tablecopy", sizeof(TGP), TB, 2, "", "kk", NULL, (SUBR)table_copy, NULL,
     NULL},
    {"tablegpw", sizeof(TGP), TB, 2, "", "k", NULL, (SUBR)table_gpw, NULL,
     NULL},
    {"tablei.a", sizeof(TABL), TR, 3, "a", "xiooo", (SUBR)tabl_setup,
     (SUBR)tableir_audio, NULL, NULL},
    {"tablei.i", sizeof(TABL), TR, 1, "i", "iiooo", (SUBR)tableir_init, NULL,
     NULL, NULL},
    {"tablei.k", sizeof(TABL), TR, 3, "k", "xiooo", (SUBR)tabl_setup,
     (SUBR)tableir_kontrol, NULL, NULL},
    {"tableicopy", sizeof(TGP), TB, 1, "", "ii", (SUBR)table_copy, NULL, NULL,
     NULL},
    {"tableigpw", sizeof(TGP), TB, 1, "", "i", (SUBR)table_gpw, NULL, NULL,
     NULL},
    {"tableikt", sizeof(TABL), TR, 3, "k", "xkooo", (SUBR)tablkt_setup,
     (SUBR)tableirkt_kontrol, NULL, NULL},
    {"tableikt.a", sizeof(TABL), TR, 3, "a", "xkooo", (SUBR)tablkt_setup,
     (SUBR)tableirkt_audio, NULL, NULL},
    {"tableimix", sizeof(TABLMIX), TB, 1, "", "iiiiiiiii", (SUBR)table_mix,
     NULL, NULL, NULL},
    {"tableiw", sizeof(TABL), TW | _QQ, 1, "", "iiiooo", (SUBR)tablew_init,
     NULL, NULL, NULL},
    {"tablekt", sizeof(TABL), TR, 3, "k", "xkooo", (SUBR)tablkt_setup,
     (SUBR)tablerkt_kontrol, NULL, NULL},
    {"tablekt.a", sizeof(TABL), TR, 3, "a", "xkooo", (SUBR)tablkt_setup,
     (SUBR)tablerkt_audio, NULL, NULL},
    {"tablemix", sizeof(TABLMIX), TB, 2, "", "kkkkkkkkk", NULL, (SUBR)table_mix,
     NULL, NULL},
    {"tableng.i", sizeof(TLEN), TR, 1, "i", "i", (SUBR)table_length, NULL, NULL,
     NULL},
    {"tableng.k", sizeof(TLEN), TR, 2, "k", "k", NULL, (SUBR)table_length, NULL,
     NULL},
    {"tablera", sizeof(TABLRA), TR, 3, "a", "kkk", (SUBR)table_ra_set,
     (SUBR)table_ra, NULL, NULL},
    {"tablew", sizeof(TABL), TW, 1, "", "iiiooo", (SUBR)tablew_init, NULL, NULL,
     NULL},
    {"tablew.aa", sizeof(TABL), TW, 3, "", "aaiooo", (SUBR)tabl_setup,
     (SUBR)tablew_audio, NULL, NULL},
    {"tablew.kk", sizeof(TABL), TW, 3, "", "kkiooo", (SUBR)tabl_setup,
     (SUBR)tablew_kontrol, NULL, NULL},
    {"tablewa", sizeof(TABLWA), TW, 3, "k", "kakp", (SUBR)table_wa_set,
     (SUBR)table_wa, NULL, NULL},
    {"tablewkt.aa", sizeof(TABL), TW, 3, "", "aakooo", (SUBR)tablkt_setup,
     (SUBR)tablewkt_audio, NULL, NULL},
    {"tablewkt.kk", sizeof(TABL), TW, 3, "", "kkkooo", (SUBR)tablkt_setup,
     (SUBR)tablewkt_kontrol, NULL, NULL},
};

LINKAGE_BUILTIN(ugtabs_localops)