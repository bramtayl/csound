...
char *strarg2name(CSOUND *csound, char *s, void *p, const char *baseName,
  ...
  if (is_string) {
    ...
    if (s == NULL) s = csound->Malloc(csound, strlen((char *)p) + 1);
    strcpy(s, (char *)p);
    ...
    if (s == NULL) s = csound->Malloc(csound, strlen(s2) + 1);
    if (*s2 == '"') s2++;
    ...
    if (i >= 0 && i <= (int)csound->strsmax && csound->strsets != NULL &&
      ...
      if (s == NULL) s = csound->Malloc(csound, strlen(csound->strsets[i]) + 1);
