...
/*
   ...
   through the linevent mechanism (linevent_public.c)
                   ...
                   int lincnt, int pcnt, CORFIL *sco)
{
    ...
    error:
      csoundMessage(csound,Str("swrite: output, sect%d line%d p%d makes"
                      " illegal reference to "),
        csound->sectcnt,lincnt,pcnt);
      while (q < p)
        csoundMessage(csound,"%c", *q++);
      while (*p != SP && *p != LF)
  ...
  /* NB np's may reference a ramp but ramps must terminate in valid nums */
{
 ...
 error1:
    ...
    goto put0;
 error2:
    csoundMessage(csound, Str("swrite: output, sect%d line%d p%d ramp "
...
/*  printf("rval=%f bp->newp2=%f prvbp->newp2-%f\n",
 ...
 error1:
    csoundMessage(csound,Str("swrite: output, sect%d line%d p%d has illegal"
                   " expramp symbol\n"),
               csound->sectcnt,lincnt,pcnt);
    goto put0;
 error2:
    csoundMessage(csound, Str("swrite: output, sect%d line%d p%d expramp "
                                "has illegal forward or backward ref\n"),
                            csound->sectcnt, lincnt, pcnt);
 put0:
    corfile_putc(csound, '0', sco);
  ...
  /* NB np's may reference a ramp but ramps must terminate in valid nums */
{
 ...
 forwrd:
    ...
    pval = stof(csound, p);     /* the error msgs generated by stof     */
    qval = stof(csound, q);                         /*   are misleading */
    rval = (MYFLT) (((double) (csoundRand31(&(csound->randSeed1)) - 1)
 ...
 error1:
    ...
    goto put0;
 error2:
    csoundMessage(csound,Str("swrite: output, sect%d line%d p%d expramp has"
...
{                             /* moves quoted ascii string to SCOREOUT file */
    ...
    corfile_putc(csound, *p++, sco);
    if (UNLIKELY(*p != SP && *p != LF)) {
      csoundMessage(csound, Str("swrite: output, sect%d line%d p%d "
...
/* CONSIDER USING SIMPLER CODE */
{
    ...
    if (UNLIKELY((*p != SP && *p != LF) || !dcnt)) {
      csoundMessage(csound,Str("swrite: output, sect%d line%d p%d has "
                                 "illegal number  "),
                      csound->sectcnt,lincnt,pcnt);
      while (q < p)
        csoundMessage(csound,"%c", *q++);
      while (*p != SP && *p != LF)
        csoundMessage(csound,"%c", *p++);
      csoundMessage(csound,Str("    String truncated\n"));
      if (!dcnt)
        corfile_putc(csound, '0', sco);
    }
    return(p);
