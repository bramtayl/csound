    ...
    if (p->done)
      *p->ktrig=FL(0.0);
      ...
      if (p->pfn != (int32)*p->kfn) {
        FUNC *ftp;
        if (UNLIKELY((ftp = csoundFTFindP(csound, p->kfn)) == NULL)) goto err1;
        p->pfn = (int32)*p->kfn;
        p->table = ftp->ftable;
      }
...
static int32_t seqtim2_set(CSOUND *csound, SEQTIM2 *p)
{
    ...
    p->pfn = (int32) *p->kfn;
    if (UNLIKELY((ftp = csoundFTnp2Find(csound, p->kfn)) == NULL)) {
      return csoundInitError(csound, Str("seqtim: incorrect table number"));
    }
...
{ "seqtime", S(SEQTIM),  TR, 3, "k",    "kkkkk", (SUBR)seqtim_set, (SUBR)seqtim, NULL, NULL},
{ "seqtime2", S(SEQTIM2),TR, 3, "k",    "kkkkkk", (SUBR)seqtim2_set, (SUBR)seqtim2, NULL, NULL}
};

ADD_INIT_FUNCTION(seqtime_localops_init, localops)
