...
static int32_t flwset(CSOUND *csound, FOL *p)
{
    ...
    if (UNLIKELY(p->length<=0L)) {           /* RWD's suggestion */
      csoundWarning(csound, Str("follow - zero length!"));
...
{ "follow",   S(FOL),   0, 3, "a",    "ai",   (SUBR)flwset,  (SUBR)follow, NULL, NULL},
{ "follow2",  S(ENV),   0, 3, "a",    "akk",  (SUBR)envset,  (SUBR)envext, NULL, NULL}
};
