...
static uintptr_t udpRecv(void *pdata)
{
    ...
    while (p->threadon) {
      ...
      if ((bytes = recvfrom(p->sock, (void *)tmp, MTU, 0, &from, &clilen)) > 0) {
        csoundWriteCircularBuffer(csound, p->cb, tmp, bytes/sizeof(MYFLT));
      }
    }
    return (uintptr_t) 0;
}
...
#ifndef _WIN32
    close(p->sock);
    csoundMessage(csound, Str("OSCraw: Closing socket\n"));
#else
    closesocket(p->sock);
    csoundMessage(csound, Str("OSCraw: Closing socket\n"));
#endif
...
static uintptr_t udpRecv_S(void *pdata)
{
    ...
    while (p->threadon) {
      ...
      if ((bytes = recvfrom(p->sock, (void *)tmp, MTU, 0, &from, &clilen)) > 0) {
        ...
        csoundWriteCircularBuffer(csound, p->cb, tmp, bytes);
      }
    }
    return (uintptr_t) 0;
}
...
static int32_t init_recv(CSOUND *csound, SOCKRECV *p)
{
    ...
    if (UNLIKELY(fcntl(p->sock, F_SETFL, O_NONBLOCK)<0))
      return csoundInitError(csound, Str("Cannot set nonblock"));
#else
    {
      u_long argp = 1;
    ...
    if (p->buffer.auxp == NULL || (uint64_t) (MTU) > p->buffer.size)
      ...
      csoundAuxAlloc(csound, MTU, &p->buffer);
    else {
    ...
    if (p->tmp.auxp == NULL || (int64_t) p->tmp.size < MTU)
      /* allocate space for the buffer */
      csoundAuxAlloc(csound, MTU, &p->tmp);
    else {
      buf = (MYFLT *) p->tmp.auxp;      /* make sure buffer is empty */
      memset(buf, 0, MTU);
    }
    p->buffsize = p->buffer.size/sizeof(MYFLT);
    p->cb = csoundCreateCircularBuffer(csound,  *p->ptr3, sizeof(MYFLT));
    /* create thread */
    p->threadon = 1;
    p->thrid = csoundCreateThread(udpRecv, (void *) p);
    csoundRegisterDeinitCallback(csound, (void *) p, deinit_udpRecv);
    p->buf = p->buffer.auxp;
...
static int32_t init_recv_S(CSOUND *csound, SOCKRECVSTR *p)
{
    ...
    /* create thread */
    p->threadon = 1;
    p->thrid = csoundCreateThread(udpRecv_S, (void *) p);
    csoundRegisterDeinitCallback(csound, (void *) p, deinit_udpRecv_S);
    p->buf = p->buffer.auxp;
    p->outsamps = p->rcvsamps = 0;
    return OK;
}
...
static int32_t send_recv_k(CSOUND *csound, SOCKRECV *p)
{
    ...
    *ksig = FL(0.0);
    if (p->outsamps >= p->rcvsamps){
      p->outsamps =  0;
      p->rcvsamps =
        csoundReadCircularBuffer(csound, p->cb, p->buf, p->buffsize);
    }
...
static int32_t send_recv_S(CSOUND *csound, SOCKRECVSTR *p)
{
    ...
    if (len>str->size) {        /* ensure enough space for result */
      str->data = mrealloc(csound, str->data, len+1);
      str->size  = len;
    }
...
static int32_t send_recv(CSOUND *csound, SOCKRECV *p)
{
    ...
    for(i=offset; i < nsmps ; i++){
      if (outsamps >= rcvsamps){
        outsamps =  0;
        rcvsamps = csoundReadCircularBuffer(csound, p->cb, buf, p->buffsize);
      }
      asig[i] = buf[outsamps];
      outsamps++;
    }
...
static int32_t init_recvS(CSOUND *csound, SOCKRECV *p)
{
    ...
    p->cb = csoundCreateCircularBuffer(csound,  *p->ptr4, sizeof(MYFLT));
    /* create thread */
    p->threadon = 1;
    p->thrid = csoundCreateThread(udpRecv, (void *) p);
    csoundRegisterDeinitCallback(csound, (void *) p, deinit_udpRecv);
...
static int32_t init_srecv(CSOUND *csound, SOCKRECVT *p)
{
    ...
    if (UNLIKELY(err<0)) {
      err= errno;
#endif
      return csoundInitError(csound, Str("bind failed (%d)"), err);
    }
    ...
    if (UNLIKELY(err<0)) {
      err= errno;
#endif
      return csoundInitError(csound, Str("listen failed (%d)"), err);
    }
    ...
    if (UNLIKELY(err<0)) {
      err= errno;
#endif
      return csoundInitError(csound, Str("accept failed (%d)"), err);
    }
    return OK;
}
...
static int32_t init_raw_osc(CSOUND *csound, RAWOSC *p)
{
    ...
    csoundRegisterDeinitCallback(csound, (void *) p, destroy_raw_osc);
    if(p->sout->data == NULL)
...
static int32_t perf_raw_osc(CSOUND *csound, RAWOSC *p) {
    ...
    if (sout->sizes[0] < 2 ||
       sout->dimensions > 1)
      return
        csoundPerfError(csound, &(p->h), Str("output array too small\n"));
    ...
    if (bytes) {
      ...
      while(size > 0 && size < MTU)  {
        ...
        if (n < sout->sizes[0]) {
          ...
          if (len >= str[n].size) {
            str[n].data = mrealloc(csound, str[n].data, len+1);
        ...
        while((c = types[j++]) != '\0' && n < sout->sizes[0]){
          if (c == 'f') {
            ...
            byteswap((char*)&f,4);
            if (str[n].size < 32) {
              str[n].data = mrealloc(csound, str[n].data, 32);
              str[n].size  = 32;
            }
...
static OENTRY sockrecv_localops[] = {
  { "sockrecv.k", S(SOCKRECV), 0, 3, "k", "ii",
    (SUBR) init_recv, (SUBR) send_recv_k, NULL, NULL},
  { "sockrecv.a", S(SOCKRECV), 0, 3, "a", "ii",
    (SUBR) init_recv, (SUBR) send_recv, NULL, NULL},
  { "sockrecv.S", S(SOCKRECVSTR), 0, 3, "S", "ii",
    (SUBR) init_recv_S,
    (SUBR) send_recv_S, NULL, NULL},
  { "sockrecvs", S(SOCKRECV), 0, 3, "aa", "ii",
    (SUBR) init_recvS,
    (SUBR) send_recvS, NULL, NULL},
  { "strecv", S(SOCKRECVT), 0, 3, "az", "Si",
    (SUBR) init_srecv,
    (SUBR) send_srecv, NULL, NULL},
