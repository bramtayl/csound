...
static int32_t midic7set(CSOUND *csound, MIDICTL2 *p)
{
    ...
    if (*p->ifn > 0) {
      if (((p->ftp = csoundFTnp2Finde(csound, p->ifn)) == NULL))
...
static int32_t midic14set(CSOUND *csound, MIDICTL3 *p)
{
    ...
    p->ctlno2 = ctlno2;
    if (*p->ifn > 0) {
      if (UNLIKELY(((p->ftp = csoundFTnp2Finde(csound, p->ifn)) == NULL)))
        p->flag = FALSE;  /* invalid ftable */
      else p->flag= TRUE;
    }
    else
...
static int32_t imidic21(CSOUND *csound, MIDICTL4 *p)
{
    ...
    if (UNLIKELY((ctlno1 = (int32)*p->ictlno1) < 0 || ctlno1 > 127 ||
      ...
      if (*p->ifn > 0) {
        /* linear interpolation routine */
        FUNC *ftp = csoundFTnp2Find(csound, p->ifn); /* gab-A1 */
        MYFLT phase, tmp, *tab;
         if (UNLIKELY(ftp == NULL))
          return csoundInitError(csound, Str("Invalid ftable no. %f"),
                                   *p->ifn);
         tab = ftp->ftable;
      /* clamp it */
      value = value >= FL(0.0) ? (value <= 1.0 ? value : FL(1.0)) : FL(0.0);
...
static int32_t ictrl14(CSOUND *csound, CTRL14 *p)
{
    ...
    if (UNLIKELY((ctlno1 = (int32)*p->ictlno1) < 0 || ctlno1 > 127 ||
      ...
      return csoundInitError(csound, Str("illegal midi channel"));
    else {
...
{                  /* (value - min) / (max - min) */
    ...
    if (UNLIKELY((fvalue = *p->ivalue) < 0. || fvalue > 1. ))
      return csoundInitError(csound, Str("value out of range"));
    else if (UNLIKELY((chan = (int32_t) *p->ichan-1) < 0 || chan > 15 ||
                      !csound->m_chnbp[chan]))
      return csoundInitError(csound, Str("illegal midi channel"));
    else
    ...
    *(p->ipgm) = FL(0.0);
    ...
    if (channelNum > 0) {
      if (UNLIKELY(channelNum > 16))
        return csoundInitError(csound, Str("invalid channel number: %d"),
                                         channelNum);
...
{ "ctrl14", 0xffff, 0, 0, "", "", NULL, NULL, NULL, NULL},
{ "ctrl21", 0xffff, 0, 0, "", "", NULL, NULL, NULL, NULL},
{ "ctrl7", 0xffff, 0, 0, "", "", NULL, NULL, NULL, NULL},
{ "midic14", 0xffff, 0, 0, "", "", NULL, NULL, NULL, NULL},
{ "midic21", 0xffff, 0, 0, "", "", NULL, NULL, NULL, NULL},
{ "midic7", 0xffff, 0, 0, "", "", NULL, NULL, NULL, NULL},
{ "midic7.i",S(MIDICTL2),0, 1,  "i", "iiio", (SUBR)imidic7, NULL, NULL, NULL},
{ "midic7.k", S(MIDICTL2),0, 3, "k", "ikko", (SUBR)midic7set, (SUBR)midic7, NULL, NULL},
{ "midic14.i", S(MIDICTL3), 0, 1,"i", "iiiio",(SUBR)imidic14, NULL, NULL, NULL},
{ "midic14.k", S(MIDICTL3), 0, 3,"k", "iikko",(SUBR)midic14set, (SUBR)midic14,NULL, NULL},
{ "midic21.i", S(MIDICTL4),0, 1,"i", "iiiiio",(SUBR)imidic21, NULL, NULL, NULL},
{ "midic21.k", S(MIDICTL4), 0, 3,"k", "iiikko",(SUBR)midic21set,(SUBR)midic21,NULL, NULL},
{ "ctrl7.i", S(CTRL7), 0, 1,    "i", "iiiio", (SUBR)ictrl7,   NULL, NULL, NULL},
{ "ctrl7.k", S(CTRL7),  0, 3,   "k", "iikko", (SUBR)ctrl7set, (SUBR)ctrl7, NULL, NULL},
{ "ctrl14.i", S(CTRL14),0, 1,   "i", "iiiiio",(SUBR)ictrl14, NULL, NULL, NULL},
{ "ctrl14.k", S(CTRL14), 0, 3,  "k", "iiikko",(SUBR)ctrl14set, (SUBR)ctrl14, NULL, NULL},
{ "ctrl21.i", S(CTRL21),0, 1,   "i", "iiiiiio", (SUBR)ictrl21, NULL, NULL, NULL},
{ "ctrl21.k", S(CTRL21), 0, 3,  "k", "iiiikko", (SUBR)ctrl21set,(SUBR)ctrl21,NULL, NULL},
{ "initc7", S(INITC7), 0, 1,     "",  "iii",  (SUBR)initc7,     NULL,     NULL, NULL},
{ "initc14", S(INITC14), 0, 1,   "",  "iiii", (SUBR)initc14,    NULL,     NULL, NULL},
{ "initc21", S(INITC21), 0, 1,   "",  "iiiii",(SUBR)initc21,    NULL,     NULL, NULL},
{ "midipgm", S(MIDIPGM_OP), 0, 1, "i", "o",   (SUBR)midipgm_opcode, NULL, NULL, NULL}
};
